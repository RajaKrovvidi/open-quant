...
from solid_rock.api.sr.assets import OqAssetApi
from solid_rock.api.sr.carbon import CarbonCard, GsCarbonApi, CarbonTargetCoverageCategory, CarbonScope, \
...
from solid_rock.api.sr.esg import ESGMeasure, GsEsgApi, ESGCard
from solid_rock.api.sr.indices import GsIndexApi
from solid_rock.api.sr.portfolios import GsPortfolioApi
from solid_rock.api.sr.reports import GsReportApi
from solid_rock.api.sr.thematics import ThematicMeasure, GsThematicApi, Region
from solid_rock.common import DateLimit, PositionType, Currency
from solid_rock.data import DataCoordinate, DataFrequency, DataMeasure
...
from solid_rock.markets.position_set import PositionSet, Position
from solid_rock.markets.report import PerformanceReport, FactorRiskReport, Report, ThematicReport, \
    flatten_results_into_df, get_thematic_breakdown_as_df
from solid_rock.markets.factor import Factor
from solid_rock.models.risk_model import MacroRiskModel, ReturnFormat, FactorType
from solid_rock.session import SrSession
from solid_rock.target.data import DataQuery
from solid_rock.target.reports import ReportStatus, ReportType
...
class Entity(metaclass=ABCMeta):
    ...
    def get(cls,
        ...
        if entity_type == 'asset':
            from solid_rock.markets.securities import SecurityMaster, AssetIdentifier
            return SecurityMaster.get_asset(id_value, AssetIdentifier.MARQUEE_ID)

        if id_type == 'MQID':
            result = SrSession.current._get(f'/{endpoint}/{id_value}')
        else:
            result = get(SrSession.current._get(f'/{endpoint}?{id_type.lower()}={id_value}'), 'results.0')
        if result:
...
class PositionedEntity(metaclass=ABCMeta):
    ...
    def _convert_pos_set_with_weights(position_set: PositionSet, currency: Currency) -> PositionSet:
        ...
        try:
            price_results = SrSession.current._post('/price/positions', payload)
