...
from solid_rock.api.sr.assets import OqAsset, OqAssetApi
from solid_rock.data import Dataset
from solid_rock.errors import MqValueError


class AssetTreeNode:
    def __init__(self, id, depth: Optional[int] = 0, date: Optional[dt.date] = None, asset: Optional[OqAsset] = None):
    ...
    def build_tree(self, dataset, underlier_column):
        ...
        if len(query) > 0:
            ...
            all_assets = OqAssetApi.get_many_assets(id=all_ids)
...
class TreeHelper:
    ...
    def __init__(self,
        ...
        self.id = id
        self.root = AssetTreeNode(self.id, 0, date, OqAssetApi.get_asset(asset_id=self.id))
