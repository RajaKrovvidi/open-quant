...
import solid_rock
from solid_rock.base import RiskKey
from solid_rock.config import DisplayOptions
from solid_rock.datetime import point_sort_order
...
class ResultInfo(metaclass=ABCMeta):
    ...
    def composition_info(components: Iterable):
        from solid_rock.markets.markets import historical_risk_key
...
class UnsupportedValue(ResultInfo):
    ...
    def _to_records(self, extra_dict, display_options: DisplayOptions = None):
        ...
        options = display_options if display_options is not None else solid_rock.config.display_options
