...
from solid_rock.analytics.common import DATAGRID_HELP_MSG
from solid_rock.analytics.common.helpers import resolve_entities, get_entity_rdate_key, get_entity_rdate_key_from_rdate, \
    get_rdate_cache_key
from solid_rock.analytics.core.processor import DataQueryInfo, MeasureQueryInfo
from solid_rock.analytics.core.processor_result import ProcessorResult
from solid_rock.analytics.core.query_helpers import aggregate_queries, fetch_query, build_query_string, valid_dimensions
from solid_rock.analytics.datagrid.data_cell import DataCell
from solid_rock.analytics.datagrid.data_column import DataColumn, ColumnFormat, MultiColumnGroup
from solid_rock.analytics.datagrid.data_row import DataRow, DimensionsOverride, ProcessorOverride, Override, \
    ValueOverride, RowSeparator
from solid_rock.analytics.datagrid.serializers import row_from_dict
from solid_rock.analytics.datagrid.utils import DataGridSort, SortOrder, SortType, DataGridFilter, FilterOperation, \
    FilterCondition, get_utc_now
from solid_rock.analytics.processors import CoordinateProcessor, EntityProcessor
from solid_rock.datetime.relative_date import RelativeDate
from solid_rock.entities.entitlements import Entitlements
from solid_rock.entities.entity import Entity
from solid_rock.errors import MqValueError
from solid_rock.session import SrSession, OAuth2Session
from solid_rock.target.common import Entitlements as Entitlements_
...
class DataGrid:
    ...
    **Usage**
    ...
    >>> from solid_rock.markets.securities import Asset, AssetIdentifier
    >>> from solid_rock.data.coordinate import DataMeasure, DataFrequency
    >>> from solid_rock.analytics.processors import LastProcessor
    >>>
    ...
    **Documentation**
    ...
    def save(self) -> str:
        ...
        if self.id_:
            response = SrSession.current._put(f'{API}/{self.id_}', datagrid_json, request_headers=DATAGRID_HEADERS)
        else:
            response = SrSession.current._post(f'{API}', datagrid_json, request_headers=DATAGRID_HEADERS)
    ...
    def delete(self):
        ...
        """
        if self.id_:
            SrSession.current._delete(f'{API}/{self.id_}', request_headers=DATAGRID_HEADERS)
        else:
    ...
    def open(self):
        ...
        domain = SrSession.current.domain.replace(".web", "")
    ...
    def _resolve_rdates(self, rule_cache: Dict = None):
        ...
        calendar = [] if not SrSession.current.is_internal() and isinstance(SrSession.current, OAuth2Session) else None
                ...
                if coord.dataset_id:
                    ...
                    try:
                        ...
                        if raw_availability is None:
                            raw_availability: Dict = SrSession.current._get(f'/data/measures/{entity_id}/availability')
