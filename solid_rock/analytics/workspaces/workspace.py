...
from solid_rock.analytics.workspaces.components import Component, TYPE_TO_COMPONENT, RelatedLink, DataGridComponent, \
    MonitorComponent, PlotComponent, DataScreenerComponent
from solid_rock.entities.entitlements import Entitlements
from solid_rock.errors import MqValueError, MqRequestError
...
class Workspace:
    ...
    def get_by_id(cls, workspace_id: str) -> 'Workspace':
        resp = SrSession.current._get(f'{API}/{workspace_id}')
        return Workspace.from_dict(resp)

    @classmethod
    def get_by_alias(cls, alias: str) -> 'Workspace':
        resp = get(SrSession.current._get(f'{API}?alias={alias}'), 'results.0')
        if not resp:
    ...
    def save(self):
        if self.__id:
            SrSession.current._put(f'{API}/{self.__id}', self.as_dict(), request_headers=HEADERS)
        elif self.__alias:
            id_ = get(SrSession.current._get(f'{API}?alias={self.__alias}'), 'results.0.id')
            if id_:
                self.__id = SrSession.current._put(f'{API}/{id_}', self.as_dict(), request_headers=HEADERS)['id']
            else:
                self.__id = SrSession.current._post(API, self.as_dict(), request_headers=HEADERS)['id']
    ...
    def create(self):
        resp = SrSession.current._post(f'{API}', self.as_dict(), request_headers=HEADERS)
        self.__id = resp['id']

    def delete(self):
        if self.__id is None:
            raise MqValueError('Workspace must have an id to be deleted.')
        resp = SrSession.current._delete(f'{API}/{self.__id}')
        self.__id = resp['id']
        ...
        for component in components:
            if isinstance(component, (WorkspaceRow, WorkspaceColumn)):
                ...
                if type_ in cls.PERSISTED_COMPONENTS:
                    try:
                        SrSession.current._delete(f'{cls.PERSISTED_COMPONENTS[type_]}/{component.id_}')
