...
from solid_rock.analytics.core.processor import MeasureQueryInfo
from solid_rock.analytics.core.processor_result import ProcessorResult
from solid_rock.data import DataFrequency
...
def fetch_query(query_info: Dict):
    ...
    try:
        if query_info['realTime'] and not query_info['range']:
            response = SrSession.current._post(f'/data/{query_info["datasetId"]}/last/query', payload=query)
        else:
            response = SrSession.current._post(f'/data/{query_info["datasetId"]}/query', payload=query)
