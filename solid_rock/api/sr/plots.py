...
from solid_rock.target.charts import Chart, ChartShare
...
class GsPlotApi:
    ...
    def get_many_charts(cls, limit: int = 100) -> Tuple[Chart, ...]:
        return SrSession.current._get('/charts?limit={limit}'.format(limit=limit), cls=Chart)['results']

    @classmethod
    def get_chart(cls, chart_id: str) -> Chart:
        return SrSession.current._get('/charts/{id}'.format(id=chart_id), cls=Chart)

    @classmethod
    def create_chart(cls, chart: Chart) -> Chart:
        return SrSession.current._post('/charts', chart, cls=Chart)

    @classmethod
    def update_chart(cls, chart: Chart):
        return SrSession.current._put('/charts/{id}'.format(id=chart.id), chart, cls=Chart)

    @classmethod
    def delete_chart(cls, chart_id: str) -> dict:
        return SrSession.current._delete('/charts/{id}'.format(id=chart_id))
    ...
    def share_chart(cls, chart_id: str, users: Iterable):
        ...
        return SrSession.current._post(f'/charts/{chart_id}/share', share, cls=Chart)
