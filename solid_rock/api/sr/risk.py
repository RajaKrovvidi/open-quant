...
from solid_rock.api.risk import RiskApi
from solid_rock.risk import RiskRequest
from solid_rock.session import SrSession
from solid_rock.target.risk import OptimizationRequest
...
class OqRiskApi(RiskApi):
    ...
    def _exec(cls, request: Union[RiskRequest, Iterable[RiskRequest]]) -> Union[Iterable, dict]:
        ...
        result, request_id = SrSession.current._post(cls.__url(request),
                                                     request,
    ...
    async def __get_results_poll(cls, responses: asyncio.Queue, results: asyncio.Queue, timeout: Optional[int] = None):
        ...
        while pending_requests or run:
            ...
            try:
                calc_results = SrSession.current._post('/risk/calculate/results/bulk', list(pending_requests.keys()))
    ...
    async def __get_results_ws(cls, responses: asyncio.Queue, results: asyncio.Queue, timeout: Optional[int] = None):
        ...
        while attempts < max_attempts:
            ...
            try:
                async with SrSession.current._connect_websocket('/risk/calculate/results/subscribe') as ws:
    ...
    def create_pretrade_execution_optimization(cls, request: OptimizationRequest) -> str:
        try:
            response = SrSession.current._post(r'/risk/execution/pretrade', request)
    ...
    def get_pretrade_execution_optimization(cls, optimization_id: str, max_attempts: int = 15):
        ...
        while attempts < max_attempts:
            ...
            try:
                results = SrSession.current._get(url)
