...
from solid_rock.target.groups import Group
...
class GsGroupsApi:
    ...
    def get_groups(cls,
        ...
        return SrSession.current._get(url, cls=Group)['results']

    @classmethod
    def create_group(cls, group: Group) -> Dict:
        return SrSession.current._post('/groups', group, cls=Group)

    @classmethod
    def get_group(cls,
                  group_id: str) -> Group:
        return SrSession.current._get(f'/groups/{group_id}', cls=Group)
    ...
    def update_group(cls,
        ...
        group_dict.pop('id')
        return SrSession.current._put(f'/groups/{group_id}', group_dict, cls=Group)

    @classmethod
    def delete_group(cls,
                     group_id: str):
        SrSession.current._delete(f'/groups/{group_id}')

    @classmethod
    def get_users_in_group(cls,
                           group_id: str) -> List:
        return SrSession.current._get(f'/groups/{group_id}/users').get('users', [])

    @classmethod
    def add_users_to_group(cls,
                           group_id: str,
                           user_ids: List[str]):
        SrSession.current._post(f'/groups/{group_id}/users', {'userIds': user_ids})

    @classmethod
    def delete_users_from_group(cls,
                                group_id: str,
                                user_ids: List[str]):
        SrSession.current._delete(f'/groups/{group_id}/users', {'userIds': user_ids})
