...
from solid_rock.target.hedge import Hedge
from solid_rock.target.hedge import PerformanceHedgeParameters, ClassificationConstraint, AssetConstraint, Target
...
class GsHedgeApi:
    ...
    def get_many_hedges(cls,
        ...
        return SrSession.current._get(url, cls=Hedge)

    @classmethod
    def create_hedge(cls, hedge: Dict) -> Hedge:
        return SrSession.current._post('/hedges', hedge, cls=Hedge)

    @classmethod
    def get_hedge(cls, hedge_id: str) -> Hedge:
        return SrSession.current._get(f'/hedges/{hedge_id}', cls=Hedge)
    ...
    def get_hedge_data(cls,
        ...
        if names:
            url += f'&name={"&name=".join(names)}'
        return SrSession.current._get(url, cls=Hedge)['results']
    ...
    def get_hedge_results(cls,
        ...
        return SrSession.current._get(url)['results'][0]

    @classmethod
    def update_hedge(cls, hedge_id: str, hedge: Hedge) -> Hedge:
        return SrSession.current._put(f'/hedges/{hedge_id}', hedge, cls=Hedge)

    @classmethod
    def delete_hedge(cls, hedge_id: str):
        return SrSession.current._delete(f'/hedges/{hedge_id}', cls=Hedge)
    ...
    def calculate_hedge(cls, hedge_query: dict) -> dict:
        ...
        """
        return SrSession.current._post('/hedges/calculations', payload=hedge_query)
