...
from solid_rock.api.data import DataApi
from solid_rock.base import Base
from solid_rock.data.core import DataContext, DataFrequency
from solid_rock.data.log import log_debug, log_warning
from solid_rock.errors import MqValueError
from solid_rock.markets import MarketDataCoordinate
from solid_rock.session import SrSession
from solid_rock.target.common import MarketDataVendor, PricingLocation, Format
from solid_rock.target.coordinates import MDAPIDataBatchResponse, MDAPIDataQuery, MDAPIDataQueryResponse, MDAPIQueryField
from solid_rock.target.data import DataQuery, DataQueryResponse, DataSetCatalogEntry
from solid_rock.target.data import DataSetEntity, DataSetFieldEntity
...
class GsDataApi(DataApi):
    ...
    def execute_query(dataset_id: str, query: Union[DataQuery, MDAPIDataQuery]):
        ...
        return SrSession.current._post('/data/{}/query'.format(dataset_id), **kwargs)
    ...
    def last_data(cls, query: Union[DataQuery, MDAPIDataQuery], dataset_id: str = None, timeout: int = None) \
        ...
        if getattr(query, 'marketDataCoordinates', None):
            result = SrSession.current._post('/data/coordinates/query/last', payload=query, **kwargs)
            return result.get('responses', ())
        else:
            result = SrSession.current._post('/data/{}/last/query'.format(dataset_id), payload=query, **kwargs)
    ...
    def get_coverage(
        ...
        body = SrSession.current._get(f'/data/{dataset_id}/coverage', payload=params)
    ...
    def create(cls, definition: Union[DataSetEntity, dict]) -> DataSetEntity:
        result = SrSession.current._post('/data/datasets', payload=definition)
        return result

    @classmethod
    def delete_dataset(cls, dataset_id: str) -> dict:
        result = SrSession.current._delete(f'/data/datasets/{dataset_id}')
        return result

    @classmethod
    def undelete_dataset(cls, dataset_id: str) -> dict:
        result = SrSession.current._put(f'/data/datasets/{dataset_id}/undelete')
        return result

    @classmethod
    def update_definition(cls, dataset_id: str, definition: Union[DataSetEntity, dict]) -> DataSetEntity:
        result = SrSession.current._put('/data/datasets/{}'.format(dataset_id), payload=definition, cls=DataSetEntity)
        return result
    ...
    def upload_data(cls, dataset_id: str, data: Union[pd.DataFrame, list, tuple]) -> dict:
        ...
        headers = None if 'us-east' in SrSession.current.domain else {'Content-Type': 'application/x-msgpack'}
        result = SrSession.current._post('/data/{}'.format(dataset_id), payload=data, request_headers=headers)
        return result
    ...
    def delete_data(cls, dataset_id: str, delete_query: Dict) -> Dict:
        ...
        """
        return SrSession.current._delete(f'/data/{dataset_id}', payload=delete_query, use_body=True)

    @classmethod
    def get_definition(cls, dataset_id: str) -> DataSetEntity:
        definition = cls.__definitions.get(dataset_id)
        if not definition:
            definition = SrSession.current._get('/data/datasets/{}'.format(dataset_id), cls=DataSetEntity)
            if not definition:
    ...
    def get_many_definitions(cls,
        ...
        body = SrSession.current._get('/data/datasets', payload=params, cls=DataSetEntity)
        results = scroll_results = body['results']
        total_results = body['totalResults']
    ...
    def get_catalog(cls,
        ...
        if len(query):
            return SrSession.current._get(f'/data/catalog?{query}', cls=DataSetCatalogEntry)['results']
        else:
            params = dict(filter(lambda item: item[1] is not None,
                                 dict(limit=limit, offset=offset, scroll=scroll, scrollId=scroll_id,
                                      enablePagination='true').items()))

            body = SrSession.current._get('/data/catalog', payload=params, cls=DataSetEntity)
            results = scroll_results = body['results']
            total_results = body['totalResults']
    ...
    def get_many_coordinates(
        ...
        results = SrSession.current._post('/data/mdapi/query', query)['results']
    ...
    def get_data_providers(cls,
        ...
        ** Usage **
        ...
        """
        response = availability if availability else SrSession.current._get(f'/data/measures/{entity_id}/availability')
    ...
    def get_market_data(cls, query, request_id=None) -> pd.DataFrame:
        SrSession.current: SrSession
        start = time.perf_counter()
        try:
            body = SrSession.current._post('/data/measures', payload=query)
    ...
    def get_types(dataset_id: str):
        results = SrSession.current._get(f'/data/catalog/{dataset_id}')
    ...
    def get_dataset_fields(
        ...
        **Examples**
        ...
        response = SrSession.current._post('/data/fields/query',
    ...
    def create_dataset_fields(
        ...
        **Examples**
        ...
        >>> from solid_rock.api.sr.data import GsDataApi
        >>> from solid_rock.target.data import DataSetFieldEntity
        >>> fields = [
        ...
        """
        params = {'fields': fields}
        response = SrSession.current._post('/data/fields/bulk', payload=params, cls=DataSetFieldEntity)
        return response['results']
    ...
    def update_dataset_fields(
        ...
        **Examples**
        ...
        """
        params = {'fields': fields}
        response = SrSession.current._put('/data/fields/bulk', payload=params, cls=DataSetFieldEntity)
        return response['results']
