...
from solid_rock.analytics.datagrid import DataGrid
from solid_rock.analytics.datagrid.datagrid import API, DATAGRID_HEADERS
...
class GsDataGridApi:
    ...
    def get_datagrids(cls, limit: int = 10, **kwargs) -> List[DataGrid]:
        raw_datagrids = get(
            SrSession.current._get(f'{API}?limit={limit}&orderBy=>lastUpdatedTime&{urllib.parse.urlencode(kwargs)}'),
            'results', [])
    ...
    def get_my_datagrids(cls, limit: int = 10, **kwargs) -> List[DataGrid]:
        user_id = SrSession.current._get('/users/self')['id']
        raw_datagrids = get(
            SrSession.current._get(
    ...
    def get_datagrid(cls, datagrid_id: str) -> DataGrid:
        raw_datagrid = SrSession.current._get(f'{API}/{datagrid_id}')
    ...
    def update_datagrid(cls, datagrid: DataGrid):
        datagrid_json = json.dumps(datagrid.as_dict())
        datagrid = SrSession.current._put(f'{API}/{datagrid.id_}', datagrid_json, request_headers=DATAGRID_HEADERS)
        return DataGrid.from_dict(datagrid)

    @classmethod
    def delete_datagrid(cls, datagrid: DataGrid):
        return SrSession.current._delete(f'{API}/{datagrid.id_}')
