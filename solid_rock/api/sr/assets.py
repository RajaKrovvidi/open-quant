...
from solid_rock.target.assets import Asset as __Asset, AssetClass, AssetType, AssetToInstrumentResponse, TemporalXRef,\
    Position, EntityQuery, PositionSet, Currency, AssetParameters
from solid_rock.target.assets import FieldFilterMap
from solid_rock.target.common import Entitlements
from solid_rock.target.reports import Report
from solid_rock.errors import MqValueError
from solid_rock.instrument import Instrument, Security
...
def _cached(fn):
    ...
    def wrapper(*args, **kwargs):
        if os.environ.get(ENABLE_ASSET_CACHING):
            ...
            k = cachetools.keys.hashkey(SrSession.current, *args, **kwargs)
            with metalock:
...
class OqAssetApi:
    ...
    def get_many_assets(
        ...
        response = SrSession.current._post('/assets/query', payload=query, cls=return_type)
    ...
    def get_many_assets_data(
        ...
        response = SrSession.current._post('/assets/data/query', payload=query)
        return response['results']
    ...
    def resolve_assets(
        ...
        return SrSession.current._post('/positions/resolver', payload=query)
    ...
    def get_asset_xrefs(
        ...
        response = SrSession.current._get('/assets/{id}/xrefs'.format(id=asset_id))
    ...
    def put_asset_xrefs(cls, asset_id: str, xrefs: List[TemporalXRef]):
        return SrSession.current._put(f'/assets/{asset_id}/xrefs', payload=xrefs)
    ...
    def get_asset(
        ...
        return SrSession.current._get('/assets/{id}'.format(id=asset_id), cls=OqAsset)

    @classmethod
    def get_asset_by_name(cls, name: str) -> OqAsset:
        ret = SrSession.current._get('/assets?name={}'.format(name))
    ...
    def create_asset(cls, asset: OqAsset) -> OqAsset:
        return SrSession.current._post('/assets', payload=asset, cls=OqAsset)

    @classmethod
    def delete_asset(cls, asset_id: str):
        return SrSession.current._delete(f'/assets/{asset_id}')

    @staticmethod
    def get_position_dates(asset_id: str) -> Tuple[dt.date, ...]:
        position_dates = SrSession.current._get(f'/assets/{asset_id}/positions/dates')['results']
    ...
    def get_asset_positions_for_dates(
        ...
        if periods > 0:
            ...
            for date in end_dates:
                ...
                try:
                    position_sets += SrSession.current._get(url)['positionSets']
    ...
    def get_or_create_asset_from_instrument(instrument: Instrument) -> str:
        ...
        results = SrSession.current._post('/assets', asset)
        return results['id']
    ...
    def get_instruments_for_asset_ids(
        ...
        instrument_infos = SrSession.current._post('/assets/instruments', asset_ids, cls=AssetToInstrumentResponse)
    ...
    def update_asset_entitlements(asset_id: str, entitlements: Entitlements) -> dict:
        ...
        try:
            results = SrSession.current._put(url, payload=entitlements)
        except HTTPError as err:
    ...
    def get_reports(cls, asset_id: str) -> Tuple[Report, ...]:
        return SrSession.current._get(f'/assets/{asset_id}/reports', cls=Report)['results']
    ...
    def map_identifiers(
        ...
        results = SrSession.current._post('/assets/data/query', payload=query)
