...
from solid_rock import version as APP_VERSION
from solid_rock.base import Base
from solid_rock.context_base import ContextBase
from solid_rock.errors import MqError, MqRequestError, MqAuthenticationError, MqUninitialisedError
from solid_rock.json_encoder import JSONEncoder, encode_default

API_VERSION = 'v1'
DEFAULT_APPLICATION = 'sr-quant'
DEFAULT_TIMEOUT = 65
...
class SrSession(ContextBase):
    ...
    def __ssl_context() -> ssl.SSLContext:
        if SrSession.__ssl_ctx is None:
            SrSession.__ssl_ctx = ssl.create_default_context(purpose=ssl.Purpose.CLIENT_AUTH)
            SrSession.__ssl_ctx.load_default_certs()
            SrSession.__ssl_ctx.load_verify_locations(certifi.where())

        return SrSession.__ssl_ctx
    ...
    def get(
        ...
        if client_id is not None:
            ...
            if is_gssso:
                try:
                    return PassThroughSRSSOSession(environment_or_domain, token, api_version=api_version,
...
class PassThroughSession(SrSession):
...
try:
    from solid_rock_auth.kerberos.session_kerberos import KerberosSessionMixin

    class KerberosSession(KerberosSessionMixin, SrSession):
        ...
        def __init__(self, environment_or_domain: str, api_version: str = API_VERSION,
            ...
            SrSession.__init__(self, domain, api_version=api_version, application=application, verify=verify,
                               http_adapter=http_adapter, application_version=application_version)

    class PassThroughSRSSOSession(KerberosSessionMixin, SrSession):
        ...
        def _authenticate(self):
            ...
            cookie = requests.cookies.create_cookie(domain='.sr.com', name='GSSSO', value=self.token)
            self._session.cookies.set_cookie(cookie)
            if self.csrf_token:
                cookie = requests.cookies.create_cookie(domain='.sr.com', name='MARQUEE-CSRF-TOKEN',
