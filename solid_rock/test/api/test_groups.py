...
from solid_rock.api.sr.groups import GsGroupsApi
...
def test_get_groups(mocker):
    ...
    response = GsGroupsApi.get_groups(ids=['id1', 'id2'],
                                      ...
                                      tags=['tag1'],
                                      limit=1)
    SrSession.current._get.assert_called_with('/groups?limit=1&offset=0&id=id1&id=id2&name=name1&tags=tag1',
                                              cls=TargetGroup)
...
def test_get_group(mocker):
    ...
    SrSession.current._get.assert_called_with('/groups/id123', cls=TargetGroup)
...
def test_create_group(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=SrSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(SrSession.current, '_post', return_value=target_group)

    # run test
    response = GsGroupsApi.create_group(target_group)
    SrSession.current._post.assert_called_with('/groups', target_group, cls=TargetGroup)
    assert response.name == 'Fake Group'
