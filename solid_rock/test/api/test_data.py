...
from solid_rock.context_base import ContextMeta
...
from solid_rock.target.data import FieldFilterMapDataQuery
from solid_rock.target.coordinates import MDAPIDataQuery
from solid_rock.target.data import MarketDataVendor, DataSetEntity, DataQuery, DataSetFieldEntity
...
def test_coordinates_data(mocker):
    ...
    # mock SrSession and data response
    mocker.patch.object(SrSession.__class__, 'default_value',
                        return_value=SrSession.get(Environment.QA, 'client_id', 'secret'))
    mocker.patch.object(SrSession.current, '_post', side_effect=[{'responses': [{'data': bond_data}]},
    ...
    SrSession.current._post.reset_mock()
    ...
    SrSession.current._post.assert_called_once_with('/data/coordinates/query',
...
def test_coordinate_data_series(mocker):
    ...
    SrSession.current._post.assert_called_with('/data/coordinates/query',
                                               payload=MDAPIDataQuery(market_data_coordinates=test_coordinates,
                                                                      start_time=start,
                                                                      end_time=end,
                                                                      vendor=MarketDataVendor.Goldman_Sachs,
...
def test_coordinate_last(mocker):
    ...
    mocker.patch.object(SrSession.__class__, 'default_value',
                        return_value=SrSession.get(Environment.QA, 'client_id', 'secret'))
    SrSession.current._post = mocker.Mock(return_value=data)
    ...
    SrSession.current._post.assert_called_once_with('/data/coordinates/query/last',
                                                    payload=MDAPIDataQuery(market_data_coordinates=test_coordinates,
                                                                           end_time=as_of,
                                                                           vendor=MarketDataVendor.Goldman_Sachs,
                                                                           format="MessagePack")
...
def test_get_coverage_api(mocker):
    ...
    mocker.patch.object(ContextMeta, 'current', return_value=SrSession(Environment.QA))
...
def test_get_many_defns_api(mocker):
    ...
    SrSession.current._get.assert_called_with('/data/datasets', payload=expected_payload, cls=DataSetEntity)
...
def test_get_many_coordinates(mocker):
    ...
    mocker.patch.object(SrSession.__class__, 'default_value',
                        return_value=SrSession.get(Environment.QA, 'client_id', 'secret'))
    SrSession.current._post = mocker.Mock(return_value={'results': coordinates})
...
def test_get_dataset_fields(mocker):
    ...
    SrSession.current._post.assert_called_once_with('/data/fields/query',
