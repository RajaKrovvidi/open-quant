...
from solid_rock.api.sr.data_screen import GsDataScreenApi
from solid_rock.session import *
from solid_rock.target.data_screen import AnalyticsScreen, FilterRequest, DataRow
...
def test_get_all_screens(mocker):
    ...
    SrSession.current._get.assert_called_with('/data/screens', cls=AnalyticsScreen)
...
def test_get_screen(mocker):
    ...
    SrSession.current._get.assert_called_with('/data/screens/{id}'.format(id=screen_id), cls=AnalyticsScreen)
...
def test_get_column_info(mocker):
    ...
    SrSession.current._get.assert_called_with('/data/screens/{id}/filters'.format(id=screen_id))
...
def test_delete_screen(mocker):
    ...
    SrSession.current._delete.assert_called_with('/data/screens/{id}'.format(id=screen_id))
...
def test_create_screen(mocker):
    ...
    SrSession.current._post.assert_called_with('/data/screens', mock_input,
...
def test_filter_screen(mocker):
    ...
    SrSession.current._post.assert_called_with('/data/screens/{id}/filter'.format(id=screen_id), mock_filter,
                                               request_headers={'Content-Type': 'application/json;charset=utf-8'},
                                               cls=DataRow)
...
def test_update_screen(mocker):
    ...
    mocker.patch.object(SrSession.__class__, 'default_value',
                        return_value=SrSession.get(Environment.QA, 'client_id', 'secret'))
    mocker.patch.object(SrSession.current, '_put', return_value=screen_input)

    # run test
    response = GsDataScreenApi.update_screen(screen_id, screen_input)
    SrSession.current._put.assert_called_with('/data/screens/{id}'.format(id=screen_id), screen_input,
                                              request_headers={'Content-Type': 'application/json;charset=utf-8'},
                                              cls=AnalyticsScreen)
