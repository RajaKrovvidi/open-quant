...
import pytest
import testfixtures
from solid_rock.api.sr.assets import OqAssetApi, OqAsset, OqTemporalXRef, ENABLE_ASSET_CACHING
from solid_rock.session import *
from solid_rock.common import PositionType
from solid_rock.target.assets import FieldFilterMap, Position, PositionSet, EntityQuery
from solid_rock.target.common import XRef
...
def test_get_asset(mocker):
    ...
    SrSession.current._get.assert_called_with('/assets/{id}'.format(id=marquee_id), cls=OqAsset)
...
def test_get_many_assets(mocker, monkeypatch):
    ...
    SrSession.current._post.assert_called_with('/assets/query', cls=OqAsset, payload=inputs)
...
def test_get_asset_xrefs(mocker):
    ...
    SrSession.current._get.assert_called_with('/assets/{id}/xrefs'.format(id=marquee_id))
...
def test_get_asset_positions_for_date(mocker):
    ...
    SrSession.current._get.assert_called_with('/assets/{id}/positions/{date}'.format(
    ...
    SrSession.current._get.assert_called_with('/assets/{id}/positions/{date}?type=close'.format(
        id=marquee_id, date=position_date))
