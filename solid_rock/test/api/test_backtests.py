...
from solid_rock.api.sr.backtests import GsBacktestApi, Backtest
from solid_rock.session import *
from solid_rock.target.common import *
...
def test_get_many_backtests(mocker):
    ...
    SrSession.current._get.assert_called_with('/backtests?limit=100', cls=Backtest)
...
def test_get_backtest(mocker):
    ...
    SrSession.current._get.assert_called_with('/backtests/{id}'.format(id=id_1), cls=Backtest)
...
def test_create_backtest(mocker):
    ...
    mocker.patch.object(SrSession.__class__, 'default_value',
                        return_value=SrSession.get(Environment.QA, 'client_id', 'secret'))
    mocker.patch.object(SrSession.current, '_post', return_value=backtest)
    ...
    SrSession.current._post.assert_called_with('/backtests', backtest, request_headers=request_headers, cls=Backtest)
...
def test_update_backtest(mocker):
    ...
    mocker.patch.object(SrSession.__class__, 'default_value',
                        return_value=SrSession.get(Environment.QA, 'client_id', 'secret'))
    mocker.patch.object(SrSession.current, '_put', return_value=backtest)

    # run test
    response = GsBacktestApi.update_backtest(backtest)
    request_headers = {'Content-Type': 'application/json;charset=utf-8', 'Accept': 'application/json;charset=utf-8'}
    SrSession.current._put.assert_called_with('/backtests/{id}'.format(id=id_1), backtest,
...
def test_delete_backtest(mocker):
    ...
    SrSession.current._delete.assert_called_with('/backtests/{id}'.format(id=id_1))
    assert response == mock_response
...
def test_schedule_backtest(mocker):
    ...
    SrSession.current._post.assert_called_with('/backtests/{id}/schedule'.format(id=id_1))
    assert response == mock_response
