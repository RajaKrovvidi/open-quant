...
from solid_rock.api.sr.base_screener import GsBaseScreenerApi
from solid_rock.session import *
from solid_rock.target.base_screener import Screener, ScreenerRow, ScreenerColumn, ScreenerParameters
...
def test_get_all_screeners(mocker):
    ...
    SrSession.current._get.assert_called_with('/data/screeners', cls=Screener)
...
def test_get_screen(mocker):
    ...
    SrSession.current._get.assert_called_with('/data/screeners/{id}'.format(id=screener_id),
                                              cls=Screener)
...
def test_create_screener(mocker):
    ...
    SrSession.current._post.assert_called_with('/data/screeners', mock_input,
...
def test_edit_screener(mocker):
    ...
    mocker.patch.object(SrSession.__class__, 'default_value',
                        return_value=SrSession.get(Environment.QA, 'client_id', 'secret'))
    mocker.patch.object(SrSession.current, '_put', return_value=mock_screen)

    # run test
    response = GsBaseScreenerApi.edit_screener(screener_id, mock_screen)
    request_headers = {'Content-Type': 'application/json;charset=utf-8'}
    SrSession.current._put.assert_called_with('/data/screeners/{id}'.format(id=screener_id),
                                              mock_screen,
                                              request_headers=request_headers,
                                              cls=Screener)
...
def test_publish_to_screener(mocker):
    ...
    request_headers = {'Content-Type': 'application/json;charset=utf-8'}
    SrSession.current._post.assert_called_with('/data/screeners/{id}/publish'.format(id=screener_id),
                                               publish_data,
                                               request_headers=request_headers)
...
def test_clear_screener(mocker):
    ...
    request_headers = {'Content-Type': 'application/json;charset=utf-8'}
    SrSession.current._post.assert_called_with('/data/screeners/{id}/clear'.format(id=screener_id),
                                               {},
                                               request_headers=request_headers)
...
def test_delete_screener(mocker):
    ...
    SrSession.current._delete.assert_called_with('/data/screeners/{id}'.format(id=screener_id))
