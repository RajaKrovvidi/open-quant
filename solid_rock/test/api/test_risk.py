...
from solid_rock.datetime.time import to_zulu_string
import solid_rock.risk as risk
from solid_rock.api.sr.risk import OqRiskApi
from solid_rock.base import Priceable
from solid_rock.common import AssetClass
from solid_rock.instrument import CommodSwap, EqForward, EqOption, FXOption, IRBasisSwap, IRSwap, IRSwaption, IRCap,\
    IRFloor
from solid_rock.markets import PricingContext
from solid_rock.session import Environment, SrSession
from solid_rock.target.risk import PricingDateAndMarketDataAsOf, RiskPosition, RiskRequestParameters, \
    OptimizationRequest
...
@mock.patch.object(OqRiskApi, '_exec')
...
def test_create_pretrade_execution_optimization():
    ...
    set_session()
    with mock.patch.object(SrSession.current, '_post') as mocker:
        mock_response = {'optimizationId': 'LI0D2ND2JCFANFAN'}
        mocker.return_value = mock_response
        response = OqRiskApi.create_pretrade_execution_optimization(request)
        SrSession.current._post.assert_called_with('/risk/execution/pretrade', request)
...
def test_get_pretrade_execution_optimization():
    ...
    set_session()
    with mock.patch.object(SrSession.current, '_get') as mocker:
        mocker.return_value = mock_response
        response = OqRiskApi.get_pretrade_execution_optimization(optimization_id)
        SrSession.current._get.assert_called_with('/risk/execution/pretrade/{}/results'.format(optimization_id))
        assert response == mock_response
