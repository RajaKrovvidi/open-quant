...
from solid_rock.base import EnumBase
from solid_rock.markets.securities import *
from solid_rock.session import *


def test_get_asset(mocker):
    marquee_id = 'MA1234567890'
    mock_response = OqAsset(asset_class=AssetClass.Equity, type_=GsAssetType.Single_Stock, name='Test Asset')
    ...
    mock_response = OqAsset(asset_class=AssetClass.Equity, type_=GsAssetType.Index, name='Test Asset')
    mocker.patch.object(SrSession.current, '_get', return_value=mock_response)
    ...
    mock_response = OqAsset(asset_class=AssetClass.Equity, type_=GsAssetType.Future, name='Test Asset')
    mocker.patch.object(SrSession.current, '_get', return_value=mock_response)
    ...
    mock_response = OqAsset(asset_class=AssetClass.Equity, type_=GsAssetType.ETF, name='Test Asset')
    mocker.patch.object(SrSession.current, '_get', return_value=mock_response)
    ...
    mock_response = OqAsset(asset_class=AssetClass.Equity, type_=GsAssetType.Custom_Basket, name='Test Asset',
                            id_=marquee_id)
    mocker.patch.object(SrSession.current, '_get', return_value=mock_response)
    ...
    mock_response = {
        'results': (OqAsset(id=marquee_id, assetClass='Equity', type='Single Stock', name='Test 1'),),
    }
...
def test_asset_types(mocker):
    ...
    asset = OqAsset(AssetClass.Equity, None, 'Test Asset')
...
def test_get_security(mocker):
    mocker.patch.object(SrSession, 'default_value', return_value=SrSession.get(Environment.QA, 'client_id', 'secret'))
    ...
    mocker.patch.object(SrSession.current, '_get', side_effect=[mock_response, mock_identifier_history_response])
...
def test_get_security_fields(mocker):
    ...
    mocker.patch.object(SrSession.current, '_get', side_effect=[mock_response, mock_identifiers_response])
...
def test_get_identifiers(mocker):
    ...
    mocker.patch.object(SrSession.current, '_get', side_effect=[assets, ids_gs, ids_ap])
    with SecMasterContext():
...
def test_get_all_identifiers(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=SrSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(SrSession.current, '_get', side_effect=[p1, p2, p3])
    with SecMasterContext():
...
def test_get_all_identifiers_with_assetTypes_not_none(mocker):
    ...
    mocker.patch.object(SrSession.current, '_get', side_effect=get_identifiers_byte)
    with SecMasterContext():
...
def test_offset_key(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=SrSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(SrSession.current, '_get', side_effect=fetch)
    with SecMasterContext():
...
def test_map_identifiers(mocker):
    ...
    mocker.patch.object(SrSession.current, '_get', side_effect=[mock2, mock2])
...
def test_map_identifiers_change(mocker):
    ...
    mocker.patch.object(SrSession.current, '_get', side_effect=[mock])
...
def test_secmaster_map_identifiers_with_passed_input_types(mocker):
    ...
    mocker.patch.object(SrSession.current, '_get',
...
def test_map_identifiers_asset_service(mocker):
    ...
    mocker.patch.object(OqAssetApi, 'map_identifiers', side_effect=lambda *arg, **kwargs: response)
    expected = {
        "2021-10-11": {
            "AAPL UN": {
                "ric": [
    ...
    mocker.patch.object(OqAssetApi, 'map_identifiers', side_effect=lambda *arg, **kwargs: {})
    with AssetContext():
        actual = SecurityMaster.map_identifiers(SecurityIdentifier.BBID,
                                                ['invalid id'],
                                                [SecurityIdentifier.RIC],
