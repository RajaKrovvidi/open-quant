...
import numpy as np
import pytest
from solid_rock.instrument import IRSwap, IRBasisSwap, IRSwaption, FXMultiCrossBinary, FXMultiCrossBinaryLeg
from solid_rock.markets import HistoricalPricingContext, PricingContext, CloseMarket
from solid_rock.markets.portfolio import Portfolio
from solid_rock.risk import MultiScenario
from solid_rock.risk import Price, RollFwd, CurveScenario, ErrorValue, DataFrameWithInfo, AggregationLevel, PnlExplain
from solid_rock.risk.core import aggregate_risk, SeriesWithInfo, FloatWithInfo
from solid_rock.risk.results import MultipleScenarioFuture
from solid_rock.risk.results import MultipleScenarioResult
from solid_rock.target.common import MarketDataPattern
...
def test_aggregation_with_diff_risk_keys(mocker):
    with MockCalc(mocker):
        ...
        with PricingContext(csa_term='EUR-OIS', visible_to_sr=True):
            r1 = portfolio1.price()
...
def test_aggregation_with_empty_measures(mocker):
    with MockCalc(mocker):
        ...
        with PricingContext(pricing_date=from_date, visible_to_sr=True):
            portfolio.resolve()
