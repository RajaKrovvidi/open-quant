...
from solid_rock.models.risk_model import FactorRiskModel, MacroRiskModel, ReturnFormat, Unit
from solid_rock.session import *
from solid_rock.target.risk_models import RiskModel as Risk_Model, RiskModelCoverage, RiskModelTerm,\
...
def mock_risk_model(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=SrSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(SrSession.current, '_post', return_value=mock_risk_model_obj)
    mocker.patch.object(SrSession.current, '_get', return_value=mock_risk_model_obj)
    mocker.patch.object(SrSession.current, '_put', return_value=mock_risk_model_obj)
...
def mock_macro_risk_model(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=SrSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(SrSession.current, '_post', return_value=mock_macro_risk_model_obj)
    mocker.patch.object(SrSession.current, '_get', return_value=mock_macro_risk_model_obj)
    mocker.patch.object(SrSession.current, '_put', return_value=mock_macro_risk_model_obj)
...
def test_update_risk_model_entitlements(mocker):
    ...
    mocker.patch.object(SrSession.current, '_get', return_value=new_model)
...
def test_get_r_squared(mocker):
    ...
    SrSession.current._post.assert_called_with('/risk/models/data/{id}/query'.format(id='macro_model_id'),
                                               query, timeout=200)
...
def test_get_predicted_beta(mocker):
    ...
    SrSession.current._post.assert_called_with('/risk/models/data/{id}/query'.format(id='model_id'),
                                               query, timeout=200)
