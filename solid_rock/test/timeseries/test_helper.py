...
import solid_rock.timeseries as ts
from solid_rock.data import DataContext, Dataset
from solid_rock.errors import MqError, MqRequestError
from unittest.mock import MagicMock
from solid_rock.session import SrSession
from solid_rock.test.api.test_thread_manager import NullContextManager
from solid_rock.timeseries.helper import _create_int_enum, _tenor_to_month, _month_to_tenor, plot_function, \
...
def test_get_df_with_retries():
    ...
    with Replace('solid_rock.timeseries.helper.RelativeDate.apply_rule', mock_apply):
...
def test_get_dataset_data_with_retries():
    replace = Replacer()
    SrSession.current = MagicMock(return_calue=NullContextManager())
    mock = replace('solid_rock.data.dataset.Dataset.get_data', Mock())
    mock.side_effect = [
