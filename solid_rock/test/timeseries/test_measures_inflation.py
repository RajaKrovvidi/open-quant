...
import solid_rock.timeseries.measures_inflation as tm
...
from solid_rock.target.common import PricingLocation, Currency as CurrEnum
from solid_rock.test.timeseries.utils import mock_request
from solid_rock.timeseries import Currency, CurrencyEnum, SecurityMaster
from solid_rock.timeseries.measures_inflation import _currency_to_tdapi_inflation_swap_rate_asset, \
    INFLATION_RATES_DEFAULTS, TdapiInflationRatesDefaultsProvider
from solid_rock.timeseries.measures_rates import _ClearingHouse
...
def test_currency_to_tdapi_inflation_swap_rate_asset(mocker):
    ...
    bbid_mock = replace('solid_rock.timeseries.measures_inflation.Asset.get_identifier', Mock())
...
def test_inflation_swap_rate(mocker):
    ...
    xrefs = replace('solid_rock.timeseries.measures.Asset.get_identifier', Mock())
    xrefs.return_value = 'GBP'
    identifiers = replace('solid_rock.timeseries.measures_inflation._get_tdapi_inflation_rates_assets', Mock())
...
def test_inflation_swap_term(mocker):
    ...
    class ObjectView(object):
        def __init__(self, d):
            self.__dict__ = d
    holidays = replace('solid_rock.datetime.GsCalendar.get', Mock(holidays=[]))
    holidays.return_value = ObjectView({'holidays': []})

    bd_calendar = replace('solid_rock.timeseries.measures_inflation._get_custom_bd', Mock())
    bd_calendar.return_value = CustomBusinessDay()
    pricing_date_mock = replace('solid_rock.timeseries.measures_inflation._range_from_pricing_date', Mock())
    ...
    market_data_mock = replace('solid_rock.timeseries.measures_inflation._get_inflation_swap_data', Mock())
    ...
    series_apply_mock = replace('solid_rock.timeseries.measures_inflation.pd.Series.apply', Mock())
