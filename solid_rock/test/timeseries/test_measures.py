...
import solid_rock.timeseries.measures as tm
import solid_rock.timeseries.measures_rates as tm_rates
from solid_rock.api.sr.assets import AssetType, OqTemporalXRef, OqAssetApi, GsIdType, IdList, OqAsset
from solid_rock.api.sr.data import GsDataApi, MarketDataResponseFrame
from solid_rock.api.sr.data import QueryType
from solid_rock.data.core import DataContext
from solid_rock.errors import MqError, MqValueError, MqTypeError
from solid_rock.json_encoder import JSONEncoder
from solid_rock.markets.baskets import Basket as CustomBasket
from solid_rock.markets.index import Index
from solid_rock.markets.securities import AssetClass, Cross, Currency, SecurityMaster, Stock, \
...
from solid_rock.session import SrSession, Environment, OAuth2Session
from solid_rock.target.common import XRef, PricingLocation, Currency as CurrEnum
from solid_rock.test.timeseries.utils import mock_request
from solid_rock.timeseries import Returns, ExtendedSeries, BenchmarkType
...
def test_currency_to_tdapi_swap_rate_asset(mocker):
    ...
    bbid_mock = replace('solid_rock.timeseries.measures.Asset.get_identifier', Mock())
    with tm.PricingContext(dt.date.today()):
...
def test_cross_stored_direction_for_fx_vol(mocker):
    mocker.patch.object(SrSession.__class__, 'default_value',
                        return_value=SrSession.get(Environment.QA, 'client_id', 'secret'))
    mocker.patch.object(SrSession.current, '_get', side_effect=mock_request)
    mocker.patch.object(SrSession.current, '_post', side_effect=mock_request)
...
def mock_fx_switch(_cls, _q, _n):
    replace = Replacer()
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_fx_empty)
    replace.restore()
...
def test_skew():
    ...
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_eq)
    ...
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_eq_norm)
    actual = tm.skew(mock_spx, '1m', tm.SkewReference.NORMALIZED, 4)
    assert_series_equal(pd.Series([2.0], index=_index, name='impliedVolatility'), pd.Series(actual))
    assert actual.dataset_ids == _test_datasets

    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_eq_spot)
    actual = tm.skew(mock_spx, '1m', tm.SkewReference.SPOT, 25)
    assert_series_equal(pd.Series([2.0], index=_index, name='impliedVolatility'), pd.Series(actual))
    assert actual.dataset_ids == _test_datasets

    mock = replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', Mock())
    mock.return_value = mock_empty_market_data_response()
    actual = tm.skew(mock_spx, '1m', tm.SkewReference.SPOT, 25)
    assert actual.empty

    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_inc)
    with pytest.raises(MqError):
...
def test_skew_fx():
    ...
    xrefs = replace('solid_rock.timeseries.measures.OqAssetApi.get_asset_xrefs', Mock())
    xrefs.return_value = [OqTemporalXRef(dt.date(2019, 1, 1), dt.date(2952, 12, 31), XRef(bbid='EURUSD', ))]
    replace('solid_rock.markets.securities.SecurityMaster.get_asset', Mock()).return_value = cross
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_fx_delta)
    mock = cross
...
def test_implied_vol():
    replace = Replacer()
    mock_spx = Index('MA890', AssetClass.Equity, 'SPX')
    replace('solid_rock.timeseries.measures.get_historical_and_last_for_measure', mock_eq_vol)
...
def test_get_last_for_measure():
    replace = Replacer()
    mock_market = replace('solid_rock.timeseries.measures._market_data_timed', Mock())
...
def test_implied_vol_no_last():
    ...
    replace('solid_rock.timeseries.measures.get_historical_and_last_for_measure', mock_eq_vol_last_empty)
    actual = tm.implied_volatility(mock_spx, '1m', tm.VolReference.DELTA_CALL, 25)
...
def test_implied_vol_fx():
    ...
    xrefs = replace('solid_rock.timeseries.measures.OqAssetApi.get_asset_xrefs', Mock())
    xrefs.return_value = [OqTemporalXRef(dt.date(2019, 1, 1), dt.date(2952, 12, 31), XRef(bbid='EURUSD', ))]
    replace('solid_rock.markets.securities.SecurityMaster.get_asset', Mock()).return_value = mock

    # for different delta strikes
    replace('solid_rock.timeseries.measures.get_historical_and_last_for_measure', mock_fx_vol)
...
def test_fx_forecast():
    ...
    xrefs.return_value = [OqTemporalXRef(dt.date(2019, 1, 1), dt.date(2952, 12, 31), XRef(bbid='EURUSD', ))]
    replace('solid_rock.markets.securities.SecurityMaster.get_asset', Mock()).return_value = mock
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_fx_forecast)
...
def test_fx_forecast_inverse():
    replace = Replacer()
    get_cross = replace('solid_rock.timeseries.measures.cross_to_usd_based_cross', Mock())
...
def test_vol_smile():
    ...
    market_mock = replace('solid_rock.timeseries.measures.get_df_with_retries', Mock())
...
def test_impl_corr_n(mocker):
    ...
    replace = Replacer()
    market_data = replace('solid_rock.timeseries.econometrics.GsDataApi.get_market_data', Mock())
    market_data.return_value = i_vol
    constituents = replace('solid_rock.timeseries.measures._get_index_constituent_weights', Mock())
    constituents.return_value = weights
    last_mock = replace('solid_rock.timeseries.measures.get_last_for_measure', Mock())
...
def test_implied_corr_basket():
    ...
    mock_asset = replace('solid_rock.timeseries.backtesting.OqAssetApi.get_many_assets_data', Mock())
    ...
    mock_data = replace('solid_rock.timeseries.get_historical_and_last_for_measure', Mock())
...
def test_real_corr_missing():
    ...
    replace = Replacer()
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', lambda *args, **kwargs: df)
    constituents = replace('solid_rock.timeseries.measures._get_index_constituent_weights', Mock())
    constituents.return_value = weights
    last_mock = replace('solid_rock.timeseries.measures.get_last_for_measure', Mock())
    last_mock.return_value = None
...
def test_cds_spreads():
    replace = Replacer()
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_cds_spread)
...
def test_avg_impl_vol(mocker):
    ...
    replace('solid_rock.api.sr.indices.GsIndexApi.get_positions_data', mock_index_positions_data)
    market_data_mock = replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', Mock())
...
def test_avg_realized_vol():
    ...
    empty_positions_data_mock = replace('solid_rock.api.sr.indices.GsIndexApi.get_positions_data', Mock())
...
def test_basis_swap_spread(mocker):
    ...
    xrefs = replace('solid_rock.timeseries.measures.Asset.get_identifier', Mock())
    xrefs.return_value = 'USD'
    identifiers = replace('solid_rock.timeseries.measures_rates._get_tdapi_rates_assets', Mock())
...
def test_swap_term_structure():
    ...
    bd_mock = replace('solid_rock.data.dataset.Dataset.get_data', Mock())
    ...
    xrefs = replace('solid_rock.timeseries.measures.Asset.get_identifier', Mock())
    xrefs.return_value = 'USD'
    identifiers_empty = replace('solid_rock.timeseries.measures.OqAssetApi.get_many_assets', Mock())
    identifiers_empty.return_value = {}
    with pytest.raises(MqValueError):
        tm_rates.swap_term_structure(**args)

    identifiers = replace('solid_rock.timeseries.measures.OqAssetApi.get_many_assets', Mock())
    ...
    pricing_date_mock = replace('solid_rock.timeseries.measures_rates._range_from_pricing_date', Mock())
    pricing_date_mock.return_value = [datetime.date(2019, 1, 1), datetime.date(2019, 1, 1)]
    bd_mock.return_value = pd.DataFrame()
    market_data_mock = replace('solid_rock.timeseries.measures_rates._market_data_timed', Mock())
...
def test_basis_swap_term_structure():
    replace = Replacer()
    range_mock = replace('solid_rock.timeseries.measures_rates._range_from_pricing_date', Mock())
...
def test_cap_floor_vol():
    ...
    identifiers = replace('solid_rock.timeseries.measures.OqAssetApi.map_identifiers', Mock())
    identifiers.return_value = {'USD-LIBOR-BBA': 'MA123'}
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_curr)
...
def test_basis():
    ...
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_cross)
...
def test_pricing_range():
    ...
    # mock
    replace = Replacer()
    cbd = replace('solid_rock.timeseries.measures._get_custom_bd', Mock())
    cbd.return_value = pd.tseries.offsets.BusinessDay()
    today = replace('solid_rock.timeseries.measures.pd.Timestamp.today', Mock())
...
def test_var_swap_tenors():
    session = SrSession.get(Environment.DEV, token='faux')

    replace = Replacer()
    get_mock = replace('solid_rock.session.SrSession._get', Mock())
...
def test_forward_var_term():
    ...
    replace = Replacer()
    market_mock = replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', Mock())
...
def test_var_swap():
    replace = Replacer()
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', _mock_var_swap_data)
...
def test_var_swap_fwd():
    ...
    # regular
    replace = Replacer()
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', _mock_var_swap_fwd)

    tenors_mock = replace('solid_rock.timeseries.measures._var_swap_tenors', Mock())
    ...
    # no data for a tenor
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', _mock_var_swap_1t)
    actual = tm.var_swap(Index('MA123', AssetClass.Equity, '123'), '1m', '1y')
    assert actual.empty
    assert actual.dataset_ids == ()
...
def _var_term_fwd():
    ...
    replace = Replacer()
    market_mock = replace('solid_rock.timeseries.measures.var_swap', Mock())
...
def test_forward_vol():
    ...
    # FX
    cross_mock = replace('solid_rock.timeseries.measures.cross_stored_direction_for_fx_vol', Mock())
...
def _skew_term_typical(reference, value, normalization_mode=None):
    ...
    replace = Replacer()
    gltsd_mock = replace('solid_rock.api.utils.ThreadPoolManager.run_async', Mock())
...
def _skew_term_typical_fx(reference, value, normalization_mode=None):
    ...
    gltsd_mock.side_effect = [[pd.DataFrame(), pd.DataFrame()], [out, out_expiry]]
    market_mock = replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', Mock())
    fx_asset_mock = replace('solid_rock.timeseries.measures.cross_stored_direction_for_fx_vol', Mock())
...
def _vol_term_empty():
    ...
    mock = replace('solid_rock.timeseries.measures.get_df_with_retries', Mock())
...
def _skew_term_empty():
    replace = Replacer()
    market_mock = replace('solid_rock.api.utils.ThreadPoolManager.run_async', Mock())
...
def test__skew_term_fetcher():
    replace = Replacer()
    market_mock = replace('solid_rock.timeseries.measures._market_data_timed', Mock())
...
def _fx_fwd_term_typical():
    ...
    thread_mock = replace('solid_rock.api.utils.ThreadPoolManager.run_async', Mock())
...
def test_bucketize_price():
    ...
    replace = Replacer()
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_commod)
    ...
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_commod_dup)
    with DataContext(datetime.date(2019, 1, 2), datetime.date(2019, 1, 2)):
        bbid_mock = replace('solid_rock.timeseries.measures.Asset.get_identifier', Mock())
        bbid_mock.return_value = 'MISO'
        with pytest.raises(ValueError):
...
def test_forward_price():
    ...
    replace = Replacer()
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_forward_price)
    ...
    with DataContext(datetime.date(2019, 1, 2), datetime.date(2019, 1, 2)):
        ...
        replace = Replacer()
        replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_missing_bucket_forward_price)
        bbid_mock = replace('solid_rock.timeseries.measures.Asset.get_identifier', Mock())
        bbid_mock.return_value = 'SPP'
        ...
        with DataContext(datetime.date(2019, 1, 2), datetime.date(2019, 1, 2)):
            ...
            ISO_mock = replace('solid_rock.timeseries.measures.Asset.get_entity', Mock())
...
def test_implied_volatility_elec():
    ...
    replace = Replacer()
    mock_get_data = replace('solid_rock.data.dataset.Dataset.get_data', Mock())
...
def test_forward_price_ng():
    ...
    replace = Replacer()
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_natgas_forward_price)
    ...
    with DataContext(datetime.date(2021, 1, 1), datetime.date(2021, 1, 1)):
        market_mock = replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_eu_natgas_forward_price)
...
def test_implied_volatility_ng():
    replace = Replacer()
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_natgas_implied_volatility)
    mock = CommodityNaturalGasHub('MA001', 'AGT')
...
def test_implied_vol_commod():
    ...
    replace = Replacer()
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_implied_volatility)
...
def test_fair_price():
    ...
    replace = Replacer()
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_fair_price)
    ...
    with pytest.raises(ValueError):
        tm.fair_price(mock,
                      tenor=None)
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_fair_price_swap)
    with DataContext(datetime.date(2019, 1, 2), datetime.date(2019, 1, 2)):
...
def test_realized_volatility():
    from solid_rock.timeseries.econometrics import volatility, Returns
    from solid_rock.timeseries.statistics import generate_series
    ...
    replace = Replacer()
    mock_data = replace('solid_rock.timeseries.measures.get_historical_and_last_for_measure', Mock())
...
def test_esg_headline_metric():
    ...
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_esg)
...
def test_rating():
    ...
    mock_aapl = Stock('MA4B66MW5E27U9VBB94', 'AAPL')
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_rating)
...
def test_fair_value(mocker):
    ...
    xrefs = replace('solid_rock.timeseries.measures.OqAssetApi.get_asset_xrefs', Mock())
    xrefs.return_value = [OqTemporalXRef(dt.date(2019, 1, 1), dt.date(2952, 12, 31), XRef(bbid='EURUSD', ))]
    asset = replace('solid_rock.markets.securities.SecurityMaster.get_asset', Mock())
    asset.return_value = mock_usdeur
    replace('solid_rock.timeseries.measures.Dataset.get_data', mock_gsdeer_gsfeer)
...
def test_factor_profile():
    ...
    mock_aapl = Stock('MA4B66MW5E27U9VBB94', 'AAPL')
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_factor_profile)
...
def test_commodity_forecast():
    ...
    replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_commodity_forecast)
...
def test_spot_carry():
    replace = Replacer()
    mock = Cross('MAA0NE9QX2ABETG6', 'USD/EUR')
    assets = replace('solid_rock.timeseries.measures.cross_stored_direction_for_fx_vol', Mock())
    ...
    with DataContext(dt.date(2020, 9, 2), dt.date(2020, 9, 4)):
        ...
        replace('solid_rock.timeseries.measures._market_data_timed', mock_fx_spot_fwd_3m)
        ...
        replace('solid_rock.timeseries.measures._market_data_timed', mock_fx_spot_fwd_2y)
...
def test_fx_implied_correlation():
    ...
    replace('solid_rock.markets.securities.SecurityMaster.get_asset', mock_get_asset)
    replace('solid_rock.markets.securities.Asset.get_identifier', cross_to_bbid)
    replace('solid_rock.timeseries.measures._market_data_timed', mock_fx_correlation)
    ...
    with DataContext(dt.date(2020, 9, 2), dt.date(2020, 9, 4)):
        ...
        replace('solid_rock.timeseries.measures._market_data_timed', lambda q, request_id=None: pd.DataFrame())
...
def test_forward_curve():
    ...
    bbid_mock = replace('solid_rock.timeseries.measures.Asset.get_identifier', Mock())
    bbid_mock.return_value = 'CAISO'
    mock_get_data = replace('solid_rock.data.dataset.Dataset.get_data_last', Mock())

    with DataContext(datetime.date(2020, 8, 20), datetime.date(2020, 9, 20)):
        # Test for term structure for bucket: peak
        replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_forward_curve_peak)
        ...
        replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_forward_curve_peak_holiday)
        ...
        replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_forward_curve_offpeak)
        ...
        replace('solid_rock.timeseries.measures.GsDataApi.get_market_data', mock_empty_forward_curve)
        mock_get_data.return_value = pd.DataFrame(data=[0], index=[datetime.date(2020, 8, 25)])
        actual = tm.forward_curve(mock_CAISO, bucket='OffPeak', market_date='20200825')
        assert actual.empty
...
def test_settlement_price():
    ...
    with DataContext(datetime.date(2021, 6, 2), datetime.date(2021, 6, 2)):
        ...
        EEX_mock = replace('solid_rock.timeseries.measures.Asset.get_entity', Mock())
        EEX_mock.return_value = {'parameters': {'exchange': 'EEX', 'productGroup': 'PowerFutures'}}
        EEX_ds = replace('solid_rock.data.dataset.Dataset.get_data', Mock())
        ...
        CC_mock = replace('solid_rock.timeseries.measures.Asset.get_entity', Mock())
        CC_mock.return_value = {'parameters': {'exchange': 'ICE', 'productGroup': 'Physical Environment'}}
        CC_ds = replace('solid_rock.data.dataset.Dataset.get_data', Mock())
        ...
        empty_mock = replace('solid_rock.timeseries.measures.Asset.get_entity', Mock())
...
def test_hloc_prices():
    ...
    replace = Replacer()
    empty_df = replace('solid_rock.timeseries.measures.Asset.get_hloc_prices', Mock())
...
def test_thematic_model_exposure():
    ...
    empty_df = replace('solid_rock.timeseries.measures.PositionedEntity.get_thematic_exposure', Mock())
    empty_df.return_value = pd.DataFrame()
    with DataContext(datetime.date(2021, 6, 2), datetime.date(2021, 6, 2)):
...
def test_thematic_model_beta():
    ...
    empty_df = replace('solid_rock.timeseries.measures.PositionedEntity.get_thematic_beta', Mock())
    empty_df.return_value = pd.DataFrame()
    with DataContext(datetime.date(2021, 6, 2), datetime.date(2021, 6, 2)):
...
def test_thematic_model_beta_single_stock():
    ...
    empty_df = replace('solid_rock.timeseries.measures.Stock.get_thematic_beta', Mock())
    empty_df.return_value = pd.DataFrame()
    with DataContext(datetime.date(2021, 6, 2), datetime.date(2021, 6, 2)):
...
def test_retail_interest_agg():
    ...
    replace = Replacer()
    positions_df = replace('solid_rock.timeseries.measures.PositionedEntity.get_positions_data', Mock())
    ...
    mock_retail = replace('solid_rock.data.dataset.Dataset.get_data', Mock())
