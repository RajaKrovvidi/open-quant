...
from solid_rock.api.sr.parser import OqParserApi
from solid_rock.api.sr.risk import OqRiskApi
from solid_rock.base import get_enum_value, InstrumentBase, Priceable, Scenario
from solid_rock.common import AssetClass, AssetType, XRef, RiskMeasure
from solid_rock.markets import HistoricalPricingContext, MarketDataCoordinate, PricingContext
from solid_rock.priceable import PriceableImpl
from solid_rock.risk import FloatWithInfo, DataFrameWithInfo, SeriesWithInfo, ResolvedInstrumentValues, \
    DEPRECATED_MEASURES
from solid_rock.risk.results import ErrorValue, MultipleRiskMeasureFuture, PricingFuture, MultipleScenarioFuture
from solid_rock.target.common import MultiScenario
...
class Instrument(PriceableImpl, InstrumentBase):
    ...
    def __asset_class_and_type_to_instrument(cls):
        if not cls.__instrument_mappings:
            import solid_rock.target.instrument as instrument_
    ...
    def calc(self, risk_measure: Union[RiskMeasure, Iterable[RiskMeasure]], fn=None) \
        ...
        **Examples**
        ...
        >>> from solid_rock.instrument import EqOption
        >>> from solid_rock.risk import EqDelta
        >>>
    ...
    def from_dict(cls, values: dict):
        ...
        if instrument is None:
            ...
            if builder_type:
                from open_quant_internal.base import decode_quill_value
    ...
    def from_quick_entry(cls, text: str, asset_class: Optional[AssetClass] = None):
        ...
        if not asset_class:
            res = OqParserApi.get_instrument_from_text(text)
            if len(res):  # multiple instruments returned
                instrument = res.pop(0)
            else:
                raise ValueError('Could not resolve instrument')
        else:
            instrument = OqParserApi.get_instrument_from_text_asset_class(text, asset_class.value)
        try:
    ...
    def from_asset_ids(cls, asset_ids: Tuple[str, ...]) -> Tuple[InstrumentBase, ...]:
        ...
        instruments = OqAssetApi.get_instruments_for_asset_ids(asset_ids)
