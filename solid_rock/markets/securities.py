...
from solid_rock.api.sr.assets import OqAsset, AssetParameters, AssetType as GsAssetType, Currency, GsIdType, OqAssetApi
from solid_rock.api.sr.data import GsDataApi
from solid_rock.api.utils import ThreadPoolManager
from solid_rock.base import get_enum_value
from solid_rock.common import DateLimit
from solid_rock.data import DataMeasure, DataFrequency, Dataset
from solid_rock.data.coordinate import DataDimensions
from solid_rock.data.coordinate import DateOrDatetime
from solid_rock.data.core import IntervalFrequency, DataAggregationOperator
from solid_rock.entities.entity import Entity, EntityIdentifier, EntityType, PositionedEntity
from solid_rock.errors import MqValueError, MqTypeError, MqRequestError
from solid_rock.json_encoder import JSONEncoder
from solid_rock.markets import PricingContext
from solid_rock.markets.indices_utils import BasketType, IndicesDatasets
from solid_rock.session import SrSession
from solid_rock.target.common import AssetClass
from solid_rock.target.data import DataQuery
...
class Asset(Entity, metaclass=ABCMeta):
    ...
    def get_url(self) -> str:
        ...
        """
        env = '-dev-ext.web' if 'dev' in get(SrSession, 'current.domain', '') else ''
        env = '-qa' if 'qa' in get(SrSession, 'current.domain', '') else env
        return f'https://marquee{env}.sr.com/s/products/{self.get_marquee_id()}/summary'
    ...
    def get_identifiers(self, as_of: dt.date = None) -> dict:
        ...
        **Examples**
        ...
        >>> sr = SecurityMaster.get_asset("GS", AssetIdentifier.TICKER)
        >>> sr.get_identifiers()

        Get identifiers as of 1Jan18:

        >>> sr.get_identifiers(dt.date(2018,1,1))
    ...
    def get_identifier(self, id_type: AssetIdentifier, as_of: dt.date = None):
        ...
        **Examples**
        ...
        >>>
        >>> sr = SecurityMaster.get_asset("GS", AssetIdentifier.TICKER)
        >>> sr.get_identifier(AssetIdentifier.SEDOL)

        Get SEDOL as of 1Jan18:

        >>> sr.get_identifier(AssetIdentifier.SEDOL, as_of=dt.date(2018,1,1))
    ...
    def get_data_series(self,
        ...
        **Examples**
        ...
        >>> from solid_rock.markets.securities import SecurityMaster
        >>> from solid_rock.data import DataMeasure
        >>>
        >>> sr = SecurityMaster.get_asset("GS", AssetIdentifier.TICKER)
        >>> sr.get_data_series(DataMeasure.CLOSE_PRICE)
...
class SecMasterAsset(Asset):
    ...
    def __load_identifiers(self) -> None:
        if self.__cached_identifiers is None:
            r = SrSession.current._get(f'/markets/securities/{self.entity["id"]}/identifiers')
            results = r['results']
...
def _get_with_retries(url, payload):
    return SrSession.current._get(url, payload=payload)
    ...
    **See also**
    ...
    def __gs_asset_to_asset(cls, gs_asset: OqAsset) -> Asset:
        ...
        if asset_type in (
            ...
            from solid_rock.markets.baskets import Basket
    ...
    def get_asset(cls,
        ...
        **Examples**
        ...
        >>> sr = SecurityMaster.get_asset("GS UN", AssetIdentifier.BLOOMBERG_ID)

        Get asset by ticker and exchange code:

        >>> sr = SecurityMaster.get_asset("GS", AssetIdentifier.TICKER, exchange_code=ExchangeCode.NYSE)
    ...
    def _get_security_master_asset(cls,
        ...
        r = SrSession.current._get('/markets/securities', payload=params)
        if r['totalResults'] == 0:
            return None
    ...
    def get_identifiers(cls, id_values: List[str], id_type: SecurityIdentifier, as_of: datetime.datetime = None,
        ...
        for k, v in id_map.items():
            r = SrSession.current._get(f'/markets/securities/{v}/identifiers')
            piece = []
            for e in r['results']:
