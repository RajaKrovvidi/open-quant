...
from solid_rock.target.common import Position as CommonPosition, PositionTag
from solid_rock.target.common import PositionSet as CommonPositionSet
from solid_rock.target.indices import PositionPriceInput
...
class PositionSet:
    ...
    def __resolve_identifiers(identifiers: List[str], date: datetime.date, **kwargs) -> Dict:
        response = OqAssetApi.resolve_assets(
    ...
    def __get_positions_data(mqids: List[str]) -> Dict:
        response = OqAssetApi.get_many_assets_data(id=mqids, fields=['id', 'name', 'bbid'])
        data = {}
