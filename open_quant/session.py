...
from open_quant import version as APP_VERSION
from open_quant.base import Base
from open_quant.context_base import ContextBase
from open_quant.errors import MqError, MqRequestError, MqAuthenticationError, MqUninitialisedError
from open_quant.json_encoder import JSONEncoder, encode_default

API_VERSION = 'v1'
DEFAULT_APPLICATION = 'oq-quant'
DEFAULT_TIMEOUT = 65
...
class OqSession(ContextBase):
    ...
    def __ssl_context() -> ssl.SSLContext:
        if OqSession.__ssl_ctx is None:
            OqSession.__ssl_ctx = ssl.create_default_context(purpose=ssl.Purpose.CLIENT_AUTH)
            OqSession.__ssl_ctx.load_default_certs()
            OqSession.__ssl_ctx.load_verify_locations(certifi.where())

        return OqSession.__ssl_ctx
    ...
    def get(
        ...
        if client_id is not None:
            ...
            if is_gssso:
                try:
                    return PassThroughOQSSOSession(environment_or_domain, token, api_version=api_version,
                                                   application=application, http_adapter=http_adapter)
                except NameError:
                    raise MqUninitialisedError('This option requires open_quant_auth to be installed')
            else:
...
class PassThroughSession(OqSession):
...
try:
    from open_quant_auth.kerberos.session_kerberos import KerberosSessionMixin

    class KerberosSession(KerberosSessionMixin, OqSession):
        ...
        def __init__(self, environment_or_domain: str, api_version: str = API_VERSION,
            ...
            OqSession.__init__(self, domain, api_version=api_version, application=application, verify=verify,
                               http_adapter=http_adapter, application_version=application_version)

    class PassThroughOQSSOSession(KerberosSessionMixin, OqSession):
        ...
        def _authenticate(self):
            ...
            cookie = requests.cookies.create_cookie(domain='.oq.com', name='GSSSO', value=self.token)
            self._session.cookies.set_cookie(cookie)
            if self.csrf_token:
                cookie = requests.cookies.create_cookie(domain='.oq.com', name='MARQUEE-CSRF-TOKEN',
