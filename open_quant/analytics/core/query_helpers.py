...
from open_quant.analytics.core.processor import MeasureQueryInfo
from open_quant.analytics.core.processor_result import ProcessorResult
from open_quant.data import DataFrequency
...
def fetch_query(query_info: Dict):
    ...
    try:
        if query_info['realTime'] and not query_info['range']:
            response = OqSession.current._post(f'/data/{query_info["datasetId"]}/last/query', payload=query)
        else:
            response = OqSession.current._post(f'/data/{query_info["datasetId"]}/query', payload=query)
