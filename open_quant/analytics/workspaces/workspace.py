...
from open_quant.analytics.workspaces.components import Component, TYPE_TO_COMPONENT, RelatedLink, DataGridComponent, \
    MonitorComponent, PlotComponent, DataScreenerComponent
from open_quant.entities.entitlements import Entitlements
from open_quant.errors import MqValueError, MqRequestError
...
class Workspace:
    ...
    def get_by_id(cls, workspace_id: str) -> 'Workspace':
        resp = OqSession.current._get(f'{API}/{workspace_id}')
        return Workspace.from_dict(resp)

    @classmethod
    def get_by_alias(cls, alias: str) -> 'Workspace':
        resp = get(OqSession.current._get(f'{API}?alias={alias}'), 'results.0')
        if not resp:
    ...
    def save(self):
        if self.__id:
            OqSession.current._put(f'{API}/{self.__id}', self.as_dict(), request_headers=HEADERS)
        elif self.__alias:
            id_ = get(OqSession.current._get(f'{API}?alias={self.__alias}'), 'results.0.id')
            if id_:
                self.__id = OqSession.current._put(f'{API}/{id_}', self.as_dict(), request_headers=HEADERS)['id']
            else:
                self.__id = OqSession.current._post(API, self.as_dict(), request_headers=HEADERS)['id']
    ...
    def create(self):
        resp = OqSession.current._post(f'{API}', self.as_dict(), request_headers=HEADERS)
        self.__id = resp['id']

    def delete(self):
        if self.__id is None:
            raise MqValueError('Workspace must have an id to be deleted.')
        resp = OqSession.current._delete(f'{API}/{self.__id}')
        self.__id = resp['id']
        ...
        for component in components:
            if isinstance(component, (WorkspaceRow, WorkspaceColumn)):
                ...
                if type_ in cls.PERSISTED_COMPONENTS:
                    try:
                        OqSession.current._delete(f'{cls.PERSISTED_COMPONENTS[type_]}/{component.id_}')
