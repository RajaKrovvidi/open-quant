...
from open_quant.analytics.common import DATAGRID_HELP_MSG
from open_quant.analytics.common.helpers import resolve_entities, get_entity_rdate_key, get_entity_rdate_key_from_rdate, \
    get_rdate_cache_key
from open_quant.analytics.core.processor import DataQueryInfo, MeasureQueryInfo
from open_quant.analytics.core.processor_result import ProcessorResult
from open_quant.analytics.core.query_helpers import aggregate_queries, fetch_query, build_query_string, valid_dimensions
from open_quant.analytics.datagrid.data_cell import DataCell
from open_quant.analytics.datagrid.data_column import DataColumn, ColumnFormat, MultiColumnGroup
from open_quant.analytics.datagrid.data_row import DataRow, DimensionsOverride, ProcessorOverride, Override, \
    ValueOverride, RowSeparator
from open_quant.analytics.datagrid.serializers import row_from_dict
from open_quant.analytics.datagrid.utils import DataGridSort, SortOrder, SortType, DataGridFilter, FilterOperation, \
    FilterCondition, get_utc_now
from open_quant.analytics.processors import CoordinateProcessor, EntityProcessor
from open_quant.datetime.relative_date import RelativeDate
from open_quant.entities.entitlements import Entitlements
from open_quant.entities.entity import Entity
from open_quant.errors import MqValueError
from open_quant.session import OqSession, OAuth2Session
from open_quant.target.common import Entitlements as Entitlements_
...
class DataGrid:
    ...
    **Usage**
    ...
    >>> from open_quant.data.coordinate import DataMeasure, DataFrequency
    >>> from open_quant.analytics.processors import LastProcessor
    >>>
    ...
    **Documentation**
    ...
    def save(self) -> str:
        ...
        if self.id_:
            response = OqSession.current._put(f'{API}/{self.id_}', datagrid_json, request_headers=DATAGRID_HEADERS)
        else:
            response = OqSession.current._post(f'{API}', datagrid_json, request_headers=DATAGRID_HEADERS)
    ...
    def delete(self):
        ...
        """
        if self.id_:
            OqSession.current._delete(f'{API}/{self.id_}', request_headers=DATAGRID_HEADERS)
        else:
    ...
    def open(self):
        ...
        domain = OqSession.current.domain.replace(".web", "")
    ...
    def _resolve_rdates(self, rule_cache: Dict = None):
        ...
        calendar = [] if not OqSession.current.is_internal() and isinstance(OqSession.current, OAuth2Session) else None
                ...
                if coord.dataset_id:
                    ...
                    try:
                        ...
                        if raw_availability is None:
                            raw_availability: Dict = OqSession.current._get(f'/data/measures/{entity_id}/availability')
