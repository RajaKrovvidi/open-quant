...
from open_quant.instrument import IRSwap, IRSwaption, CurveScenario
from open_quant.markets import HistoricalPricingContext, PricingContext, BackToTheFuturePricingContext, \
    historical_risk_key
from open_quant.markets.portfolio import Portfolio
from open_quant.risk.results import PortfolioPath, PortfolioRiskResult
from open_quant.session import Environment, OqSession
from open_quant.target.common import PositionSet
from open_quant.target.portfolios import Portfolio as MarqueePortfolio
from open_quant.test.utils.mock_calc import MockCalc


def set_session():
    from open_quant.session import OAuth2Session
    OAuth2Session.init = mock.MagicMock(return_value=None)
    OqSession.use(Environment.PROD, 'client_id', 'secret')
...
def test_get_instruments(mocker):
    ...
    mocker.patch.object(GsPortfolioApi, 'get_portfolio', return_value=(MarqueePortfolio(id_='id',
                                                                                        name='name',
                                                                                        currency='USD')))
    mocker.patch.object(OqAssetApi, 'get_instruments_for_positions', return_value=([]))
