...
from open_quant.api.oq.assets import AssetClass, AssetType, OqAsset, OqAssetApi
...
from open_quant.entities.entitlements import User
from open_quant.errors import MqError
from open_quant.markets.baskets import Basket, ErrorMessage
from open_quant.markets.indices_utils import ReturnType
from open_quant.markets.position_set import Position, PositionSet
from open_quant.session import OqSession, Environment
from open_quant.target.common import Entitlements as TargetEntitlements, \
    PositionSet as TargetPositionSet, Position as TargetPosition, ReportParameters, XRef
from open_quant.target.indices import CustomBasketsResponse, CustomBasketRiskParams
from open_quant.target.reports import Report, User as TargetUser
...
gs_asset = OqAsset(asset_class=AssetClass.Equity,
...
@mock.patch.object(OqSession.__class__, 'default_value')
def mock_session(mocker):
    """ Mock OqSession helper """
    mocker.return_value = OqSession.get(Environment.QA, 'client_id', 'secret')
...
def mock_basket_init(mocker, user: Dict, existing: bool = True):
    ...
    if existing:
        mock_response(mocker, OqAssetApi, 'resolve_assets', resolved_asset)
        mock_response(mocker, OqAssetApi, 'get_asset', gs_asset)
        mock_response(mocker, OqAssetApi, 'get_latest_positions', target_position_set)
        mock_response(mocker, OqAssetApi, 'get_many_assets_data', assets_data)
...
def test_basket_update_entitlements(mocker):
    ...
    mock_response(mocker, OqAssetApi, 'update_asset_entitlements', entitlements_response)
    response = basket.update()
    OqAssetApi.update_asset_entitlements.assert_called()
