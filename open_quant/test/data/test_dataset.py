...
from open_quant.target.data import Format
...
def test_query_data(mocker):
    mocker.patch("open_quant.api.oq.data.GsDataApi.query_data", return_value=test_data)
    mocker.patch("open_quant.api.oq.data.GsDataApi.get_types", return_value=test_types)
...
def test_last_data(mocker):
    mocker.patch("open_quant.api.oq.data.GsDataApi.last_data", return_value=[test_data[-1]])
    mocker.patch("open_quant.api.oq.data.GsDataApi.get_types", return_value=test_types)
    dataset = Dataset(Dataset.TR.TREOD)
...
def test_get_coverage(mocker):
    mocker.patch("open_quant.api.oq.data.GsDataApi.get_coverage", return_value=test_coverage_data)
    mocker.patch("open_quant.api.oq.data.GsDataApi.get_types", return_value={'gsid': 'string'})
...
def test_data_series_format(mocker):
    ...
    mocker.patch.object(OqSession.current, '_post', side_effect=lambda *args, **kwargs: mock_response)
    mocker.patch.object(GsDataApi, 'symbol_dimensions', return_value=('assetId',))
    mocker.patch("open_quant.api.oq.data.GsDataApi.get_types", return_value=test_types)

    actual = Dataset('TREOD').get_data_series(field='tradePrice', start=start, end=end, assetId='MA4B66MW5E27U8P32SB')
    pd.testing.assert_series_equal(actual, expected)
    assert len(OqSession.current._post.mock_calls) == 1
    name, args, kwargs = OqSession.current._post.mock_calls[0]
