...
from open_quant.api.oq.risk_models import GsRiskModelApi, GsFactorRiskModelApi
from open_quant.session import *
from open_quant.target.risk_models import RiskModel, Factor, RiskModelCalendar, \
...
def test_get_risk_models(mocker):
    ...
    OqSession.current._get.assert_called_with('/risk/models?', cls=RiskModel)
...
def test_get_risk_model(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_get', return_value=model)

    # run test
    response = GsRiskModelApi.get_risk_model(model_id)
    OqSession.current._get.assert_called_with('/risk/models/{id}'.format(id=model_id), cls=RiskModel)
    assert response == expected_response
...
def test_create_risk_model(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=model)

    # run test
    response = GsRiskModelApi.create_risk_model(model)
    OqSession.current._post.assert_called_with('/risk/models', model, cls=RiskModel)
    assert response == expected_response
...
def test_update_risk_model(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_put', return_value=model)

    # run test
    response = GsRiskModelApi.update_risk_model(model)
    OqSession.current._put.assert_called_with('/risk/models/{id}'.format(id='WW_TEST_MODEL'), model, cls=RiskModel)
...
def test_delete_risk_model(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_delete', return_value=mock_response)

    # run test
    response = GsRiskModelApi.delete_risk_model('model id')
    OqSession.current._delete.assert_called_with('/risk/models/{id}'.format(id='model id'))
...
def test_get_risk_model_calendar(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_get', return_value=calendar)

    # run test
    response = GsRiskModelApi.get_risk_model_calendar('id')
    OqSession.current._get.assert_called_with('/risk/models/{id}/calendar'.format(id='id'), cls=RiskModelCalendar)
...
def test_upload_risk_model_calendar(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_put', return_value=calendar)

    # run test
    response = GsRiskModelApi.upload_risk_model_calendar('WW_TEST_MODEL', calendar)
    OqSession.current._put.assert_called_with('/risk/models/{id}/calendar'.format(id='WW_TEST_MODEL'),
...
def test_get_risk_model_factors(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_get', return_value=factors)

    # run test
    response = GsFactorRiskModelApi.get_risk_model_factors(model_id='id')
    OqSession.current._get.assert_called_with('/risk/models/id/factors', cls=Factor)
    assert response == expected_response
...
def test_create_risk_model_factor(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=factor)

    # run test
    response = GsFactorRiskModelApi.create_risk_model_factor(model_id='id', factor=factor)
    OqSession.current._post.assert_called_with('/risk/models/id/factors', factor, cls=Factor)
    assert response == expected_response
...
def test_update_risk_model_factor(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_put', return_value=factor)

    # run test
    response = GsFactorRiskModelApi.update_risk_model_factor(model_id='id', factor=factor)
    OqSession.current._put.assert_called_with('/risk/models/{id}/factors/{identifier}'
...
def test_get_risk_model_coverage(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=results)

    # run test
    response = GsFactorRiskModelApi.get_risk_model_coverage()
    OqSession.current._post.assert_called_with('/risk/models/coverage', {}, timeout=200)
...
def test_upload_risk_model_data(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value='Successfully uploaded')

    # run test
    response = GsFactorRiskModelApi.upload_risk_model_data(model_id='id', model_data=risk_model_data)
    OqSession.current._post.assert_called_with('/risk/models/data/{id}'.format(id='id'), risk_model_data, timeout=200)
...
def test_upload_macro_risk_model_data(mocker):
    ...
    OqSession.current._post.assert_called_with(
...
def test_get_risk_model_data(mocker):
    ...
    OqSession.current._post.assert_called_with('/risk/models/data/{id}/query'.format(id='id'), query, timeout=200)
...
def test_get_r_squared(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=r_squared_results)
...
def test_get_fair_value_gap(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=fvg_results)
...
def test_get_factor_standard_deviation(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=factor_standard_deviation_results)
...
def test_get_factor_z_score(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=factor_z_score_results)
...
def test_get_predicted_beta(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=predicted_beta_results)
...
def test_get_global_predicted_beta(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=global_predicted_beta_results)
...
def test_get_daily_return(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=daily_return_results)
...
def test_get_specific_return(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=specific_return_resutls)
