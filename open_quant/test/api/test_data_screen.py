...
from open_quant.api.oq.data_screen import GsDataScreenApi
from open_quant.session import *
from open_quant.target.data_screen import AnalyticsScreen, FilterRequest, DataRow
...
def test_get_all_screens(mocker):
    ...
    OqSession.current._get.assert_called_with('/data/screens', cls=AnalyticsScreen)
...
def test_get_screen(mocker):
    ...
    OqSession.current._get.assert_called_with('/data/screens/{id}'.format(id=screen_id), cls=AnalyticsScreen)
...
def test_get_column_info(mocker):
    ...
    OqSession.current._get.assert_called_with('/data/screens/{id}/filters'.format(id=screen_id))
...
def test_delete_screen(mocker):
    ...
    OqSession.current._delete.assert_called_with('/data/screens/{id}'.format(id=screen_id))
...
def test_create_screen(mocker):
    ...
    OqSession.current._post.assert_called_with('/data/screens', mock_input,
...
def test_filter_screen(mocker):
    ...
    OqSession.current._post.assert_called_with('/data/screens/{id}/filter'.format(id=screen_id), mock_filter,
                                               request_headers={'Content-Type': 'application/json;charset=utf-8'},
                                               cls=DataRow)
...
def test_update_screen(mocker):
    ...
    mocker.patch.object(OqSession.__class__, 'default_value',
                        return_value=OqSession.get(Environment.QA, 'client_id', 'secret'))
    mocker.patch.object(OqSession.current, '_put', return_value=screen_input)

    # run test
    response = GsDataScreenApi.update_screen(screen_id, screen_input)
    OqSession.current._put.assert_called_with('/data/screens/{id}'.format(id=screen_id), screen_input,
                                              request_headers={'Content-Type': 'application/json;charset=utf-8'},
                                              cls=AnalyticsScreen)
