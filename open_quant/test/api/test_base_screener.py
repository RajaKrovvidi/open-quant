...
from open_quant.api.oq.base_screener import GsBaseScreenerApi
from open_quant.session import *
from open_quant.target.base_screener import Screener, ScreenerRow, ScreenerColumn, ScreenerParameters
...
def test_get_all_screeners(mocker):
    ...
    OqSession.current._get.assert_called_with('/data/screeners', cls=Screener)
...
def test_get_screen(mocker):
    ...
    OqSession.current._get.assert_called_with('/data/screeners/{id}'.format(id=screener_id),
                                              cls=Screener)
...
def test_create_screener(mocker):
    ...
    OqSession.current._post.assert_called_with('/data/screeners', mock_input,
...
def test_edit_screener(mocker):
    ...
    mocker.patch.object(OqSession.__class__, 'default_value',
                        return_value=OqSession.get(Environment.QA, 'client_id', 'secret'))
    mocker.patch.object(OqSession.current, '_put', return_value=mock_screen)

    # run test
    response = GsBaseScreenerApi.edit_screener(screener_id, mock_screen)
    request_headers = {'Content-Type': 'application/json;charset=utf-8'}
    OqSession.current._put.assert_called_with('/data/screeners/{id}'.format(id=screener_id),
                                              mock_screen,
                                              request_headers=request_headers,
                                              cls=Screener)
...
def test_publish_to_screener(mocker):
    ...
    request_headers = {'Content-Type': 'application/json;charset=utf-8'}
    OqSession.current._post.assert_called_with('/data/screeners/{id}/publish'.format(id=screener_id),
                                               publish_data,
                                               request_headers=request_headers)
...
def test_clear_screener(mocker):
    ...
    request_headers = {'Content-Type': 'application/json;charset=utf-8'}
    OqSession.current._post.assert_called_with('/data/screeners/{id}/clear'.format(id=screener_id),
                                               {},
                                               request_headers=request_headers)
...
def test_delete_screener(mocker):
    ...
    OqSession.current._delete.assert_called_with('/data/screeners/{id}'.format(id=screener_id))
