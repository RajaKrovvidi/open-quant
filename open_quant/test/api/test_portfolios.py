...
from open_quant.api.oq.portfolios import GsPortfolioApi, Portfolio, PositionSet
from open_quant.session import *
from open_quant.target.common import Position
...
def test_get_many_portfolios(mocker):
    ...
    OqSession.current._get.assert_called_with('/portfolios?&limit=100', cls=Portfolio)
...
def test_get_portfolio(mocker):
    ...
    OqSession.current._get.assert_called_with('/portfolios/{id}'.format(id=id_1), cls=Portfolio)
...
def test_create_portfolio(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=portfolio)

    # run test
    response = GsPortfolioApi.create_portfolio(portfolio)
    OqSession.current._post.assert_called_with('/portfolios', portfolio, cls=Portfolio)
...
def test_update_portfolio(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_put', return_value=portfolio)

    # run test
    response = GsPortfolioApi.update_portfolio(portfolio)
    OqSession.current._put.assert_called_with('/portfolios/{id}'.format(id=id_1), portfolio, cls=Portfolio)
    assert response == portfolio
...
def test_delete_portfolio(mocker):
    ...
    OqSession.current._delete.assert_called_with('/portfolios/{id}'.format(id=id_1))
    assert response == mock_response
...
def test_get_portfolio_positions(mocker):
    ...
    OqSession.current._get.assert_called_with(
...
def test_get_portfolio_position_dates(mocker):
    ...
    OqSession.current._get.assert_called_with('/portfolios/{id}/positions/dates'.format(id=id_1))
...
def test_get_risk_models_by_coverage(mocker):
    ...
    OqSession.current._get.assert_called_with('/portfolios/portfolio_id/models?sortByTerm=Medium')
