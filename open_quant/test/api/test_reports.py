...
from open_quant.target.reports import Report, ReportJob, ReportParameters
...
def test_get_reports(mocker):
    ...
    OqSession.current._get.assert_called_with('/reports?limit=100', cls=Report)
...
def test_get_report(mocker):
    ...
    OqSession.current._get.assert_called_with('/reports/{id}'.format(id=id_1), cls=Report)
...
def test_create_report(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=report)

    # run test
    response = GsReportApi.create_report(report)
    OqSession.current._post.assert_called_with('/reports', report, cls=Report)
...
def test_update_report(mocker):
    ...
    mocker.patch.object(
        ...
        return_value=OqSession.get(
            ...
            'client_id',
            'secret'))
    mocker.patch.object(OqSession.current, '_put', return_value=report)

    # run test
    response = GsReportApi.update_report(report)
    OqSession.current._put.assert_called_with('/reports/{id}'.format(id=id_1), report, cls=Report)
    assert response == report
...
def test_delete_portfolio(mocker):
    ...
    OqSession.current._delete.assert_called_with('/reports/{id}'.format(id=id_1))
    assert response == mock_response
...
def test_get_report_status(mocker):
    ...
    OqSession.current._get.assert_called_with('/reports/{id}/status'.format(id=id_1))
    assert response == mock_response
...
def test_get_report_jobs(mocker):
    ...
    OqSession.current._get.assert_called_with('/reports/{id}/jobs'.format(id=id_1))
    assert response == expected_response
...
def test_report_job(mocker):
    ...
    OqSession.current._get.assert_called_with('/reports/jobs/{id}'.format(id=id_1))
    assert response == mock_response
...
def test_cancel_report_job(mocker):
    ...
    OqSession.current._post.assert_called_with('/reports/jobs/{id}/cancel'.format(id=id_1))
    assert response == mock_response
...
def test_update_report_job(mocker):
    ...
    OqSession.current._post.assert_called_with('/reports/jobs/{id}/update'.format(id=id_1), status_body)
    assert response == mock_response
...
def test_get_factor_risk_report_results(mocker):
    ...
    OqSession.current._get.assert_called_with('/risk/factors/reports/reportId/results?')
    assert response == mock_response
...
def test_get_factor_risk_report_view(mocker):
    ...
    OqSession.current._get.assert_called_with(f'/risk/factors/reports/{report_id}/views?view=Risk')
    assert response == mock_response
