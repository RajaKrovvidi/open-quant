...
from open_quant.datetime.time import to_zulu_string
import open_quant.risk as risk
from open_quant.api.oq.risk import OqRiskApi
from open_quant.base import Priceable
from open_quant.common import AssetClass
from open_quant.instrument import CommodSwap, EqForward, EqOption, FXOption, IRBasisSwap, IRSwap, IRSwaption, IRCap,\
    IRFloor
from open_quant.markets import PricingContext
from open_quant.session import Environment, OqSession
from open_quant.target.risk import PricingDateAndMarketDataAsOf, RiskPosition, RiskRequestParameters, \
    OptimizationRequest
...
def test_create_pretrade_execution_optimization():
    ...
    set_session()
    with mock.patch.object(OqSession.current, '_post') as mocker:
        mock_response = {'optimizationId': 'LI0D2ND2JCFANFAN'}
        mocker.return_value = mock_response
        response = OqRiskApi.create_pretrade_execution_optimization(request)
        OqSession.current._post.assert_called_with('/risk/execution/pretrade', request)
...
def test_get_pretrade_execution_optimization():
    ...
    set_session()
    with mock.patch.object(OqSession.current, '_get') as mocker:
        mocker.return_value = mock_response
        response = OqRiskApi.get_pretrade_execution_optimization(optimization_id)
        OqSession.current._get.assert_called_with('/risk/execution/pretrade/{}/results'.format(optimization_id))
        assert response == mock_response
