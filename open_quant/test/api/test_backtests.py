...
from open_quant.api.oq.backtests import GsBacktestApi, Backtest
from open_quant.session import *
from open_quant.target.common import *
...
def test_get_many_backtests(mocker):
    ...
    OqSession.current._get.assert_called_with('/backtests?limit=100', cls=Backtest)
...
def test_get_backtest(mocker):
    ...
    OqSession.current._get.assert_called_with('/backtests/{id}'.format(id=id_1), cls=Backtest)
...
def test_create_backtest(mocker):
    ...
    mocker.patch.object(OqSession.__class__, 'default_value',
                        return_value=OqSession.get(Environment.QA, 'client_id', 'secret'))
    mocker.patch.object(OqSession.current, '_post', return_value=backtest)
    ...
    OqSession.current._post.assert_called_with('/backtests', backtest, request_headers=request_headers, cls=Backtest)
...
def test_update_backtest(mocker):
    ...
    mocker.patch.object(OqSession.__class__, 'default_value',
                        return_value=OqSession.get(Environment.QA, 'client_id', 'secret'))
    mocker.patch.object(OqSession.current, '_put', return_value=backtest)

    # run test
    response = GsBacktestApi.update_backtest(backtest)
    request_headers = {'Content-Type': 'application/json;charset=utf-8', 'Accept': 'application/json;charset=utf-8'}
    OqSession.current._put.assert_called_with('/backtests/{id}'.format(id=id_1), backtest,
...
def test_delete_backtest(mocker):
    ...
    OqSession.current._delete.assert_called_with('/backtests/{id}'.format(id=id_1))
    assert response == mock_response
...
def test_schedule_backtest(mocker):
    ...
    OqSession.current._post.assert_called_with('/backtests/{id}/schedule'.format(id=id_1))
    assert response == mock_response
