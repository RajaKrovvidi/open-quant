...
from open_quant.timeseries import VolReference
from open_quant.timeseries.backtesting import Basket, basket_series, MqValueError, MqTypeError, RebalFreq, date, \
    DataContext, np
...
def test_basket_price():
    ...
    mock_data = replace('open_quant.timeseries.backtesting.ts.get_historical_and_last_for_measure', Mock())
...
def test_basket_average_implied_vol():
    ...
    mock_spot = replace('open_quant.timeseries.backtesting.ts.get_historical_and_last_for_measure', Mock())
    mock_spot.side_effect = [implied_vol.rename(columns={'impliedVolatility': 'spot'})]

    mock_data = replace('open_quant.api.utils.ThreadPoolManager.run_async', Mock())
...
def test_basket_average_realized_vol():
    ...
    mock_get_last = replace('open_quant.timeseries.measures.get_last_for_measure', Mock())
...
def test_basket_average_realized_vol_wts():
    ...
    mock_vol = replace('open_quant.timeseries.backtesting.volatility', Mock())
...
def test_basket_average_realized_vol_intraday():
    ...
    mock_today = replace('open_quant.timeseries.get_last_for_measure', Mock())
