...
import open_quant.timeseries.measures_inflation as tm
import open_quant.timeseries.measures as tm_rates
from open_quant.api.oq.assets import OqAsset
from open_quant.api.oq.data import GsDataApi, MarketDataResponseFrame
from open_quant.data import DataContext
from open_quant.errors import MqError, MqValueError
from open_quant.session import OqSession, Environment
from open_quant.target.common import PricingLocation, Currency as CurrEnum
from open_quant.test.timeseries.utils import mock_request
from open_quant.timeseries import Currency, CurrencyEnum, SecurityMaster
from open_quant.timeseries.measures_inflation import _currency_to_tdapi_inflation_swap_rate_asset, \
    INFLATION_RATES_DEFAULTS, TdapiInflationRatesDefaultsProvider
from open_quant.timeseries.measures_rates import _ClearingHouse
...
def test_currency_to_tdapi_inflation_swap_rate_asset(mocker):
    ...
    bbid_mock = replace('open_quant.timeseries.measures_inflation.Asset.get_identifier', Mock())
...
def test_get_tdapi_inflation_rates_assets(mocker):
    mock_asset_1 = OqAsset(asset_class='Rates', id='MA26QSMPX9990G66', type_='InflationSwap', name='Test_asset')
    mock_asset_2 = OqAsset(asset_class='Rates', id='MA44SBCHF192S6FR', type_='InflationSwap', name='Test_asset')
...
def test_inflation_swap_rate(mocker):
    ...
    xrefs = replace('open_quant.timeseries.measures.Asset.get_identifier', Mock())
    xrefs.return_value = 'GBP'
    identifiers = replace('open_quant.timeseries.measures_inflation._get_tdapi_inflation_rates_assets', Mock())
...
def test_inflation_swap_term(mocker):
    ...
    class ObjectView(object):
        def __init__(self, d):
            self.__dict__ = d
    holidays = replace('open_quant.datetime.GsCalendar.get', Mock(holidays=[]))
    holidays.return_value = ObjectView({'holidays': []})

    bd_calendar = replace('open_quant.timeseries.measures_inflation._get_custom_bd', Mock())
    bd_calendar.return_value = CustomBusinessDay()
    pricing_date_mock = replace('open_quant.timeseries.measures_inflation._range_from_pricing_date', Mock())
    ...
    market_data_mock = replace('open_quant.timeseries.measures_inflation._get_inflation_swap_data', Mock())
    ...
    series_apply_mock = replace('open_quant.timeseries.measures_inflation.pd.Series.apply', Mock())
