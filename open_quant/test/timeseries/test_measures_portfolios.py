...
from open_quant.target.reports import Report, ReportParameters
import open_quant.timeseries.measures_portfolios as mp
...
def test_financial_conditions_index():
    ...
    mock = replace('open_quant.markets.report.PerformanceReport.get_pnl', Mock())
...
def test_custom_aum():
    ...
    # mock GsPortfolioApi.get_reports()
    mock = replace('open_quant.api.oq.portfolios.GsPortfolioApi.get_custom_aum', Mock())
    mock.return_value = df
    with DataContext(datetime.date(2020, 1, 1), datetime.date(2019, 1, 3)):
        actual = mp.aum('MP1')
        assert actual.index.equals(idx)
