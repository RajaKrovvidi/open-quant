...
import open_quant.timeseries as ts
from open_quant.data import DataContext, Dataset
from open_quant.errors import MqError, MqRequestError
from unittest.mock import MagicMock
from open_quant.session import OqSession
from open_quant.test.api.test_thread_manager import NullContextManager
from open_quant.timeseries.helper import _create_int_enum, _tenor_to_month, _month_to_tenor, plot_function, \
...
def test_get_df_with_retries():
    ...
    with Replace('open_quant.timeseries.helper.RelativeDate.apply_rule', mock_apply):
...
def test_get_dataset_data_with_retries():
    replace = Replacer()
    OqSession.current = MagicMock(return_calue=NullContextManager())
    mock = replace('open_quant.data.dataset.Dataset.get_data', Mock())
    mock.side_effect = [
