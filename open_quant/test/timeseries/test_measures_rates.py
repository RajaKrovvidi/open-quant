...
from open_quant.api.oq.assets import OqTemporalXRef, OqAssetApi
from open_quant.api.oq.data import MarketDataResponseFrame, QueryType
from open_quant.data import Fields, Dataset
from open_quant.data.core import DataContext
from open_quant.errors import MqValueError, MqError
from open_quant.markets.securities import Cross, Currency
from open_quant.session import OqSession, Environment
from open_quant.target.common import PricingLocation, XRef
from open_quant.test.timeseries.test_measures import _test_datasets, map_identifiers_default_mocker, \
    mock_empty_market_data_response
from open_quant.test.timeseries.utils import mock_request
from open_quant.timeseries import TdapiRatesDefaultsProvider, SWAPTION_DEFAULTS, CurrencyEnum, SecurityMaster, \
    ExtendedSeries
from open_quant.timeseries.measures_rates import _swaption_build_asset_query, _currency_to_tdapi_swaption_rate_asset, \
...
def test_currency_to_tdapi_swaption_rate_asset_retuns_throws():
    replace = Replacer()
    replace('open_quant.timeseries.measures.Asset.get_identifier', Mock()).return_value = "ZAR"
...
def test_swaption_swaption_vol_term2_returns_data():
    ...
    replace('open_quant.timeseries.measures.Asset.get_identifier', Mock()).return_value = "GBP"
    replace('open_quant.timeseries.measures_rates._get_tdapi_rates_assets', Mock(), Mock()).return_value = [
        "MADWG3WHCKNE1DJA", "MAH6JK3TZJJGFQ65"]
    replace('open_quant.timeseries.measures_rates._range_from_pricing_date', Mock(), Mock()).return_value = [
        dt.date(2020, 1, 2), dt.date(2020, 1, 2)]
    replace('open_quant.timeseries.measures_rates._market_data_timed', Mock()).return_value = df
...
def test_swaption_premium_return_data():
    ...
    replace('open_quant.timeseries.measures.Asset.get_identifier', Mock()).return_value = "GBP"
    replace('open_quant.timeseries.measures_rates._get_tdapi_rates_assets', Mock(),
...
def test_cross_to_fxfwd_xcswp_asset(mocker):
    replace = Replacer()
    mocker.patch.object(OqSession.__class__, 'current',
                        return_value=OqSession.get(Environment.DEV, 'client_id', 'secret'))
    mocker.patch.object(OqSession.current, '_get', side_effect=mock_request)
    mocker.patch.object(SecurityMaster, 'get_asset', side_effect=mock_request)
    bbid_mock = replace('open_quant.timeseries.measures.Asset.get_identifier', Mock())
    correct_mapping = {
...
def test_policy_rate_term_structure(mocker):
    ...
    with DataContext(dt.date(2019, 12, 6), dt.date(2019, 12, 6)):
        ...
        xrefs.return_value = [OqTemporalXRef(dt.date(2019, 1, 1), dt.date(2952, 12, 31), XRef(bbid='EUR', ))]
        ...
        mock_get_data = replace('open_quant.timeseries.measures_rates.policy_rate_term_structure_rt', Mock())
        mock_get_data.return_value = pd.DataFrame()
...
def test_policy_rate_expectation(mocker):
    ...
    with DataContext(dt.date(2019, 12, 6), dt.date(2019, 12, 6)):
        ...
        mock_get_data = replace('open_quant.timeseries.measures_rates.policy_rate_expectation_rt', Mock())
        mock_get_data.return_value = pd.DataFrame()
...
def test_policy_rate_term_structure_rt(mocker):
    ...
    with DataContext(dt.date(2020, 2, 14), dt.date(2020, 3, 14)):
        ...
        cbd = replace('open_quant.timeseries.measures._get_custom_bd', Mock())
        cbd.return_value = pd.tseries.offsets.BusinessDay()
        replace('open_quant.timeseries.measures_rates._get_tdapi_rates_assets', get_cb_swap_assets_mocker)
...
def test_get_swap_from_meeting_date(mocker):
    ...
    replace = Replacer()
    cbd = replace('open_quant.timeseries.measures_rates.get_cb_meeting_swap', Mock())
...
def test_policy_rate_expectation_rt(mocker):
    ...
    with DataContext(dt.date(2020, 2, 14), dt.date(2020, 3, 14)):
        ...
        cbd = replace('open_quant.timeseries.measures._get_custom_bd', Mock())
        cbd.return_value = pd.tseries.offsets.BusinessDay()
        mock_swap = replace('open_quant.timeseries.measures_rates._get_tdapi_rates_assets', Mock())
        mock_swap.return_value = 'MA000SPOT'

        mock_swap = replace('open_quant.timeseries.measures_rates._get_swap_from_meeting_date', Mock())
