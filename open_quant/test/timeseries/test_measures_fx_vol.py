...
import open_quant.timeseries.measures_fx_vol as tm_fxo
import open_quant.timeseries.measures_xccy as tm
from open_quant.api.oq.assets import OqAsset
from open_quant.api.oq.data import GsDataApi, MarketDataResponseFrame, QueryType
from open_quant.errors import MqError, MqValueError
from open_quant.session import OqSession, Environment
from open_quant.target.common import PricingLocation
from open_quant.test.timeseries.utils import mock_request
from open_quant.timeseries import Currency, Cross, Bond, CurrencyEnum, SecurityMaster
from open_quant.timeseries.measures_fx_vol import _currencypair_to_tdapi_fxo_asset, _currencypair_to_tdapi_fxfwd_asset
...
def test_currencypair_to_tdapi_fxo_asset(mocker):
    ...
    bbid_mock = replace('open_quant.timeseries.measures_fx_vol.Asset.get_identifier', Mock())
...
def test_cross_stored_direction_for_fx_vol():
    ...
    assets = replace('open_quant.markets.securities.SecurityMaster.get_asset', Mock())
...
def test_fx_vol_measure(mocker):
    ...
    xrefs = replace('open_quant.timeseries.measures.Asset.get_identifier', Mock())
    xrefs.return_value = 'GBPUSD'
    identifiers = replace('open_quant.timeseries.measures_fx_vol._get_tdapi_fxo_assets', Mock())
...
def test_fx_vol_measure_legacy(mocker):
    ...
    mocker.patch.object(tm_rates, '_extract_series_from_df',
                        return_value=expected)
    xrefs = replace('open_quant.timeseries.measures.cross_stored_direction_for_fx_vol', Mock())
    ...
    xrefs = replace('open_quant.timeseries.measures.Asset.get_identifier', Mock())
    xrefs.return_value = 'GBPUSD'
    xrefs = replace('open_quant.timeseries.measures._cross_stored_direction_helper', Mock())
    xrefs.return_value = 'GBPUSD'
    ...
    xrefs = replace('open_quant.timeseries.measures_fx_vol._cross_stored_direction_helper', Mock())
    xrefs.return_value = 'GBPUSD'
    assets = replace('open_quant.markets.securities.SecurityMaster.get_asset', Mock())
    assets.return_value = mock_gbp
    actual = tm_fxo.implied_volatility_fxvol(**args)
    ...
    args['relative_strike'] = 100
    xrefs = replace('open_quant.timeseries.measures_fx_vol._preprocess_implied_vol_strikes_fx', Mock())
