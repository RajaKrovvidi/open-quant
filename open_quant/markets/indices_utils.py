...
from open_quant.api.oq.assets import AssetClass, OqAssetApi
...
from open_quant.datetime.date import prev_business_date
...
def get_my_baskets(user_id: str = None) -> Optional[pd.DataFrame]:
    ...
    **Examples**
    ...
    >>> get_my_baskets()
    """
    user_id = user_id if user_id is not None else OqSession.current.client_id
    ...
    if len(response):
        ...
        for row_group in row_groups:
            ...
            baskets = OqAssetApi.get_many_assets_data(id=entity_ids, fields=['id', 'ticker', 'name', 'liveDate'])
...
def get_flagship_baskets(fields: List[str] = [],
    ...
    **See also**
    ...
    response = OqAssetApi.get_many_assets_data_scroll(**query, as_of=as_of, limit=2000, scroll='1m')
...
def get_flagships_with_assets(identifiers: List[str],
    ...
    **See also**
    ...
    fields = list(set(fields).union(set(['id', 'name', 'ticker', 'region', 'type', 'description',
                                         'styles', 'liveDate', 'assetClass'])))
    response = OqAssetApi.resolve_assets(identifier=identifiers, fields=['id'], limit=1)
...
def get_flagships_constituents(fields: List[str] = [],
    ...
    **See also**
    ...
    coverage_results = ThreadPoolManager.run_async([
        partial(OqAssetApi.get_many_assets_data_scroll, **query, limit=2000, scroll='1m'),
    ...
    asset_data = OqAssetApi.get_many_assets_data_scroll(id=asset_ids, fields=fields, limit=2000, scroll='1m')
