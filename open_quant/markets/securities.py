...
from open_quant.api.oq.assets import OqAsset, AssetParameters, AssetType as GsAssetType, Currency, GsIdType, OqAssetApi
from open_quant.api.oq.data import GsDataApi
from open_quant.api.utils import ThreadPoolManager
from open_quant.base import get_enum_value
from open_quant.common import DateLimit
from open_quant.data import DataMeasure, DataFrequency, Dataset
from open_quant.data.coordinate import DataDimensions
from open_quant.data.coordinate import DateOrDatetime
from open_quant.data.core import IntervalFrequency, DataAggregationOperator
from open_quant.entities.entity import Entity, EntityIdentifier, EntityType, PositionedEntity
from open_quant.errors import MqValueError, MqTypeError, MqRequestError
from open_quant.json_encoder import JSONEncoder
from open_quant.markets import PricingContext
from open_quant.markets.indices_utils import BasketType, IndicesDatasets
from open_quant.session import OqSession
from open_quant.target.common import AssetClass
from open_quant.target.data import DataQuery
...
class Asset(Entity, metaclass=ABCMeta):
    ...
    def get_url(self) -> str:
        ...
        """
        env = '-dev-ext.web' if 'dev' in get(OqSession, 'current.domain', '') else ''
        env = '-qa' if 'qa' in get(OqSession, 'current.domain', '') else env
        return f'https://marquee{env}.oq.com/s/products/{self.get_marquee_id()}/summary'
    ...
    def get_identifiers(self, as_of: dt.date = None) -> dict:
        ...
        **Examples**
        ...
        >>> oq = SecurityMaster.get_asset("GS", AssetIdentifier.TICKER)
        >>> oq.get_identifiers()

        Get identifiers as of 1Jan18:

        >>> oq.get_identifiers(dt.date(2018,1,1))
        ...
        **See also**
        ...
        xrefs = OqAssetApi.get_asset_xrefs(self.get_marquee_id())
        identifiers = {}
    ...
    def get_identifier(self, id_type: AssetIdentifier, as_of: dt.date = None):
        ...
        **Examples**
        ...
        >>>
        >>> oq = SecurityMaster.get_asset("GS", AssetIdentifier.TICKER)
        >>> oq.get_identifier(AssetIdentifier.SEDOL)

        Get SEDOL as of 1Jan18:

        >>> oq.get_identifier(AssetIdentifier.SEDOL, as_of=dt.date(2018,1,1))
    ...
    def get_data_series(self,
        ...
        **Examples**
        ...
        >>> from open_quant.markets.securities import SecurityMaster
        >>> from open_quant.data import DataMeasure
        >>>
        >>> oq = SecurityMaster.get_asset("GS", AssetIdentifier.TICKER)
        >>> oq.get_data_series(DataMeasure.CLOSE_PRICE)
...
class SecMasterAsset(Asset):
    ...
    def __load_identifiers(self) -> None:
        if self.__cached_identifiers is None:
            r = OqSession.current._get(f'/markets/securities/{self.entity["id"]}/identifiers')
            results = r['results']
...
class Stock(Asset):
    ...
    def get_thematic_beta(self,
        ...
        response = OqAssetApi.resolve_assets(identifier=[basket_identifier],
...
def _get_with_retries(url, payload):
    return OqSession.current._get(url, payload=payload)
...
class SecurityMaster:
    ...
    **See also**
    ...
    @classmethod
    def __gs_asset_to_asset(cls, gs_asset: OqAsset) -> Asset:
    ...
    def get_asset(cls,
        ...
        **Examples**
        ...
        >>> oq = SecurityMaster.get_asset("GS UN", AssetIdentifier.BLOOMBERG_ID)

        Get asset by ticker and exchange code:

        >>> oq = SecurityMaster.get_asset("GS", AssetIdentifier.TICKER, exchange_code=ExchangeCode.NYSE)
        ...
        **See also**
        ...
        if id_type is AssetIdentifier.MARQUEE_ID:
            gs_asset = OqAssetApi.get_asset(id_value)
        ...
        if sort_by_rank:
            results = OqAssetApi.get_many_assets(as_of=as_of, return_type=dict, order_by=['>rank'], **query)
            result = get(results, '0')

            if result:
                result = OqAsset.from_dict(result)
        else:
            results = OqAssetApi.get_many_assets(as_of=as_of, **query)
    ...
    def _get_security_master_asset(cls,
        ...
        r = OqSession.current._get('/markets/securities', payload=params)
        if r['totalResults'] == 0:
            return None
    ...
    def get_identifiers(cls, id_values: List[str], id_type: SecurityIdentifier, as_of: datetime.datetime = None,
        ...
        for k, v in id_map.items():
            r = OqSession.current._get(f'/markets/securities/{v}/identifiers')
            piece = []
            for e in r['results']:
    ...
    def map_identifiers(
        ...
        **Examples**
        ...
        if cls._source == SecurityMasterSource.ASSET_SERVICE:
            ...
            result = OqAssetApi.map_identifiers(input_type, output_type, list(ids), as_of=as_of, multimap=True)
            if len(result) == 0:
                return result
