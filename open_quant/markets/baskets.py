...
from open_quant.data.fields import DataMeasure
from open_quant.entities.entity import EntityType, PositionedEntity
from open_quant.entities.entitlements import Entitlements as BasketEntitlements
from open_quant.errors import MqError, MqValueError
from open_quant.json_encoder import JSONEncoder
from open_quant.markets.indices_utils import *
from open_quant.markets.securities import Asset, AssetType as SecAssetType
from open_quant.session import OqSession
from open_quant.target.data import DataQuery
from open_quant.target.indices import *
from open_quant.target.reports import Report, ReportStatus
from open_quant.markets.position_set import PositionSet
...
class Basket(Asset, PositionedEntity):
    ...
    """
    def __init__(self, gs_asset: OqAsset = None, **kwargs):
    ...
    def create(self) -> Dict:
        ...
        **See also**
        ...
        gs_asset = OqAssetApi.get_asset(response.asset_id)
    ...
    def update(self) -> Dict:
        ...
        **See also**
        ...
        if not entitlements == self.__initial_entitlements:
            response = OqAssetApi.update_asset_entitlements(self.id, entitlements)
        ...
        gs_asset = OqAssetApi.get_asset(self.id)
        self.__latest_create_report = GsReportApi.get_report(response.report_id)
        self.__init__(gs_asset=gs_asset, _finish_init=True)
        return response.as_dict()
    ...
    def get_corporate_actions(self,
        ...
        **Examples**
        ...
        >>> from open_quant.markets.baskets import Basket
        >>> from open_quant.markets.indices_utils import CorporateActionType
        >>>
        >>> basket = Basket.get("GSMBXXXX")
    ...
    def get_url(self) -> str:
        ...
        **Examples**
        ...
        >>> basket.get_url()
        """
        env = '-dev' if 'dev' in get(OqSession, 'current.domain', '') else ''
        env = '-qa' if 'qa' in get(OqSession, 'current.domain', '') else env
        return f'https://marquee{env}.oq.com/s/products/{self.id}/summary'
    ...
    def parent_basket(self) -> Optional[str]:
        ...
        if has(self, '__clone_parent_id') and not has(self, '__parent_basket'):
            self.__parent_basket = get(OqAssetApi.get_asset(self.__clone_parent_id), 'id')
    ...
    def __finish_initialization(self):
        ...
        if has(self, 'id'):
            if not has(self, '__initial_positions'):
                position_set = OqAssetApi.get_latest_positions(self.id, PositionType.ANY)
    ...
    @staticmethod
    def __get_gs_asset(identifier: str) -> OqAsset:
        """ Resolves basket identifier during initialization """
        response = OqAssetApi.resolve_assets(identifier=[identifier], fields=['id'], limit=1)[identifier]
        if len(response) == 0 or get(response, '0.id') is None:
            raise MqValueError(f'Basket could not be found using identifier {identifier}')
        return OqAssetApi.get_asset(get(response, '0.id'))
    ...
    def __populate_current_attributes_for_existing_basket(self, gs_asset: OqAsset):
