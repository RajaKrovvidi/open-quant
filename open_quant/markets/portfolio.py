...
from open_quant.base import InstrumentBase
from open_quant.common import RiskMeasure
from open_quant.instrument import Instrument, AssetType
from open_quant.markets import HistoricalPricingContext, OverlayMarket, PricingContext, PositionContext
from open_quant.priceable import PriceableImpl
from open_quant.risk import ResolvedInstrumentValues
from open_quant.risk.results import CompositeResultFuture, PortfolioRiskResult, PortfolioPath, PricingFuture
from open_quant.target.common import RiskPosition
from open_quant.target.portfolios import Portfolio as MarqueePortfolio
from open_quant.target.portfolios import Position, PositionSet, RiskRequest, PricingDateAndMarketDataAsOf
...
class Portfolio(PriceableImpl):
    ...
    def from_asset_id(asset_id: str, date=None):
        asset = OqAssetApi.get_asset(asset_id)
        response = OqAssetApi.get_asset_positions_for_date(asset_id, date) if date else \
            OqAssetApi.get_latest_positions(asset_id)
        ...
        instruments = OqAssetApi.get_instruments_for_positions(positions)
    ...
    def from_asset_name(name: str):
        asset = OqAssetApi.get_asset_by_name(name)
    ...
    def save(self, overwrite: Optional[bool] = False):
        ...
        position_set = PositionSet(
            ...
            positions=tuple(Position(asset_id=OqAssetApi.get_or_create_asset_from_instrument(i))
