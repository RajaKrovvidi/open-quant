...
from open_quant.api.oq.assets import OqAssetApi
from open_quant.api.oq.carbon import CarbonCard, GsCarbonApi, CarbonTargetCoverageCategory, CarbonScope, \
...
from open_quant.api.oq.esg import ESGMeasure, GsEsgApi, ESGCard
from open_quant.api.oq.indices import GsIndexApi
from open_quant.api.oq.portfolios import GsPortfolioApi
from open_quant.api.oq.reports import GsReportApi
from open_quant.api.oq.thematics import ThematicMeasure, GsThematicApi, Region
from open_quant.common import DateLimit, PositionType, Currency
from open_quant.data import DataCoordinate, DataFrequency, DataMeasure
from open_quant.data.coordinate import DataDimensions
from open_quant.entities.entitlements import Entitlements
...
from open_quant.markets.position_set import PositionSet, Position
from open_quant.markets.report import PerformanceReport, FactorRiskReport, Report, ThematicReport, \
    flatten_results_into_df, get_thematic_breakdown_as_df
from open_quant.markets.factor import Factor
from open_quant.models.risk_model import MacroRiskModel, ReturnFormat, FactorType
from open_quant.session import OqSession
from open_quant.target.data import DataQuery
from open_quant.target.reports import ReportStatus, ReportType
...
class Entity(metaclass=ABCMeta):
    ...
    def get(cls,
        ...
        if entity_type == 'asset':
            from open_quant.markets.securities import SecurityMaster, AssetIdentifier
            return SecurityMaster.get_asset(id_value, AssetIdentifier.MARQUEE_ID)

        if id_type == 'MQID':
            result = OqSession.current._get(f'/{endpoint}/{id_value}')
        else:
            result = get(OqSession.current._get(f'/{endpoint}?{id_type.lower()}={id_value}'), 'results.0')
        if result:
...
class PositionedEntity(metaclass=ABCMeta):
    ...
    def get_entitlements(self) -> Entitlements:
        if self.positioned_entity_type == EntityType.PORTFOLIO:
            ...
            response = OqAssetApi.get_asset(self.id)
        else:
    ...
    def get_latest_position_set(self,
        ...
        if self.positioned_entity_type == EntityType.ASSET:
            response = OqAssetApi.get_latest_positions(self.id, position_type)
    ...
    def get_position_set_for_date(self,
                                  ...
                                  position_type: PositionType = PositionType.CLOSE) -> PositionSet:
        if self.positioned_entity_type == EntityType.ASSET:
            response = OqAssetApi.get_asset_positions_for_date(self.id, date, position_type)
            if len(response) == 0:
    ...
    def get_position_sets(self,
        ...
        if self.positioned_entity_type == EntityType.ASSET:
            response = OqAssetApi.get_asset_positions_for_dates(self.id, start, end, position_type)
            if len(response) == 0:
    ...
    def _convert_pos_set_with_weights(position_set: PositionSet, currency: Currency) -> PositionSet:
        ...
        try:
            price_results = OqSession.current._post('/price/positions', payload)
    ...
    def get_position_dates(self) -> Tuple[dt.date, ...]:
        ...
        if self.positioned_entity_type == EntityType.ASSET:
            return OqAssetApi.get_position_dates(asset_id=self.id)
        raise NotImplementedError
    ...
    def get_reports(self) -> List[Report]:
        if self.positioned_entity_type == EntityType.PORTFOLIO:
            ...
            reports_as_target = OqAssetApi.get_reports(asset_id=self.id)
        else:
            raise NotImplementedError
        report_objects = []
