...
import open_quant
from open_quant.base import RiskKey
from open_quant.config import DisplayOptions
from open_quant.datetime import point_sort_order
...
class ResultInfo(metaclass=ABCMeta):
    ...
    def composition_info(components: Iterable):
        from open_quant.markets.markets import historical_risk_key
...
class UnsupportedValue(ResultInfo):
    ...
    def _to_records(self, extra_dict, display_options: DisplayOptions = None):
        ...
        options = display_options if display_options is not None else open_quant.config.display_options
...
def aggregate_risk(results: Iterable[Union[DataFrameWithInfo, Future]], threshold: Optional[float] = None) \
    ...
    **Examples**
    ...
    >>> from open_quant.instrument import IRCap, IRFloor
    >>> from open_quant.markets import PricingContext
    >>> from open_quant.risk import IRDelta, IRVega
    >>>
    >>> cap = IRCap('5y', 'GBP')
...
def subtract_risk(left: DataFrameWithInfo, right: DataFrameWithInfo) -> pd.DataFrame:
    ...
    **Examples**
    ...
    >>> from open_quant.datetime.date import business_day_offset
    >>> from open_quant.instrument IRSwap
    >>> from open_quant.markets import PricingContext
