...
from open_quant.target.hedge import Hedge
from open_quant.target.hedge import PerformanceHedgeParameters, ClassificationConstraint, AssetConstraint, Target
...
class GsHedgeApi:
    ...
    def get_many_hedges(cls,
        ...
        return OqSession.current._get(url, cls=Hedge)

    @classmethod
    def create_hedge(cls, hedge: Dict) -> Hedge:
        return OqSession.current._post('/hedges', hedge, cls=Hedge)

    @classmethod
    def get_hedge(cls, hedge_id: str) -> Hedge:
        return OqSession.current._get(f'/hedges/{hedge_id}', cls=Hedge)
    ...
    def get_hedge_data(cls,
        ...
        if names:
            url += f'&name={"&name=".join(names)}'
        return OqSession.current._get(url, cls=Hedge)['results']
    ...
    def get_hedge_results(cls,
        ...
        return OqSession.current._get(url)['results'][0]

    @classmethod
    def update_hedge(cls, hedge_id: str, hedge: Hedge) -> Hedge:
        return OqSession.current._put(f'/hedges/{hedge_id}', hedge, cls=Hedge)

    @classmethod
    def delete_hedge(cls, hedge_id: str):
        return OqSession.current._delete(f'/hedges/{hedge_id}', cls=Hedge)
    ...
    def calculate_hedge(cls, hedge_query: dict) -> dict:
        ...
        """
        return OqSession.current._post('/hedges/calculations', payload=hedge_query)
