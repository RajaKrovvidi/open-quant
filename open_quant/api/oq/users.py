...
class GsUsersApi:
    ...
    def get_users(cls,
        ...
        return OqSession.current._get(f'{url}&limit={limit}&offset={offset}', cls=User)['results']

    @classmethod
    def get_my_guid(cls) -> str:
        return f"guid:{OqSession.current._get('/users/self')['id']}"
    ...
    def get_current_user_info(cls) -> List[str]:
        ...
        :return: user
        """
        return OqSession.current._get('/users/self')

    @classmethod
    def get_current_app_managers(cls) -> List[str]:
        return [f"guid:{manager}" for manager in get(OqSession.current._get('/users/self'), 'appManagers', [])]
