...
from open_quant.target.risk_models import RiskModel, RiskModelCalendar, Factor, RiskModelData, \
...
class GsRiskModelApi:
    ...
    def create_risk_model(cls, model: RiskModel) -> RiskModel:
        return OqSession.current._post('/risk/models', model, cls=RiskModel)

    @classmethod
    def get_risk_model(cls, model_id: str) -> RiskModel:
        return OqSession.current._get(f'/risk/models/{model_id}', cls=RiskModel)
    ...
    def get_risk_models(cls,
        ...
        return OqSession.current._get(url, cls=RiskModel)['results']

    @classmethod
    def update_risk_model(cls, model: RiskModel) -> RiskModel:
        return OqSession.current._put(f'/risk/models/{model.id}', model, cls=RiskModel)

    @classmethod
    def delete_risk_model(cls, model_id: str) -> Dict:
        return OqSession.current._delete(f'/risk/models/{model_id}')

    @classmethod
    def get_risk_model_calendar(cls, model_id: str) -> RiskModelCalendar:
        return OqSession.current._get(f'/risk/models/{model_id}/calendar', cls=RiskModelCalendar)

    @classmethod
    def upload_risk_model_calendar(cls, model_id: str, model_calendar: RiskModelCalendar) -> RiskModelCalendar:
        return OqSession.current._put(f'/risk/models/{model_id}/calendar', model_calendar, cls=RiskModelCalendar)
...
class GsFactorRiskModelApi(GsRiskModelApi):
    ...
    def get_risk_model_factors(cls, model_id: str) -> Tuple[Factor, ...]:
        return OqSession.current._get(f'/risk/models/{model_id}/factors', cls=Factor)['results']

    @classmethod
    def create_risk_model_factor(cls, model_id: str, factor: Factor) -> Factor:
        return OqSession.current._post(f'/risk/models/{model_id}/factors', factor, cls=Factor)

    @classmethod
    def get_risk_model_factor(cls, model_id: str, factor_id: str) -> Factor:
        return OqSession.current._get(f'/risk/models/{model_id}/factors/{factor_id}')

    @classmethod
    def update_risk_model_factor(cls, model_id: str, factor: Factor) -> Factor:
        url = f'/risk/models/{model_id}/factors/{factor.identifier}'
        return OqSession.current._put(url, factor, cls=Factor)

    @classmethod
    def delete_risk_model_factor(cls, model_id: str, factor_id: str) -> Dict:
        return OqSession.current._delete(f'/risk/models/{model_id}/factors/{factor_id}')
    ...
    def get_risk_model_coverage(cls,
        ...
        return OqSession.current._post('/risk/models/coverage', query, timeout=200)['results']
    ...
    def upload_risk_model_data(cls,
        ...
        return OqSession.current._post(url, model_data, timeout=200)
    ...
    def get_risk_model_data(cls, model_id: str, start_date: dt.date, end_date: dt.date = None,
        ...
        return OqSession.current._post(f'/risk/models/data/{model_id}/query', query, timeout=200)
