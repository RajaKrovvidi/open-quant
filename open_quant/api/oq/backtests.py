...
from open_quant.target.backtests import *
...
class GsBacktestApi:
    ...
    def get_many_backtests(cls,
        ...
        return OqSession.current._get('/backtests?{query}'.format(query=query_string), cls=Backtest)['results']

    @classmethod
    def get_backtest(cls, backtest_id: str) -> Backtest:
        return OqSession.current._get('/backtests/{id}'.format(id=backtest_id), cls=Backtest)
    ...
    def create_backtest(cls, backtest: Backtest) -> Backtest:
        ...
        return OqSession.current._post('/backtests', backtest, request_headers=request_headers, cls=Backtest)

    @classmethod
    def update_backtest(cls, backtest: Backtest):
        request_headers = {'Content-Type': 'application/json;charset=utf-8', 'Accept': 'application/json;charset=utf-8'}
        return OqSession.current._put('/backtests/{id}'.format(id=backtest.id), backtest,
                                      request_headers=request_headers,
                                      cls=Backtest)

    @classmethod
    def delete_backtest(cls, backtest_id: str) -> dict:
        return OqSession.current._delete('/backtests/{id}'.format(id=backtest_id))

    @classmethod
    def get_results(cls, backtest_id: str) -> Tuple[BacktestResult, ...]:
        return OqSession.current._get('/backtests/results?id={id}'.format(id=backtest_id))['backtestResults']
    ...
    def get_comparison_results(
        ...
        result = OqSession.current._get('/backtests/results?{query}'.format(query=query_string))
        return result['backtestResults'], result['comparisonResults']

    @classmethod
    def schedule_backtest(cls, backtest_id: str) -> dict:
        return OqSession.current._post('/backtests/{id}/schedule'.format(id=backtest_id))
    ...
    def run_backtest(cls, backtest: Backtest, correlation_id: str = None) -> BacktestResult:
        ...
        response = OqSession.current._post('/backtests/calculate', backtest, request_headers=request_headers)
    ...
    def calculate_position_risk(cls, backtestRiskRequest: BacktestRiskRequest) -> dict:
        request_headers = {'Content-Type': 'application/json;charset=utf-8', 'Accept': 'application/json;charset=utf-8'}
        return OqSession.current._post('/backtests/calculate-position-risk', backtestRiskRequest,
                                       request_headers=request_headers)

    @classmethod
    def get_ref_data(cls) -> BacktestRefData:
        return OqSession.current._get('/backtests/refData', cls=BacktestRefData)

    @classmethod
    def update_ref_data(cls, backtest_ref_data: BacktestRefData):
        request_headers = {'Content-Type': 'application/json;charset=utf-8', 'Accept': 'application/json;charset=utf-8'}
        return OqSession.current._put('/backtests/refData', backtest_ref_data,
                                      request_headers=request_headers,
