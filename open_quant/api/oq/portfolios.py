...
from open_quant.common import RiskRequest, Currency
from open_quant.instrument import Instrument
from open_quant.session import OqSession
from open_quant.target.portfolios import Portfolio, Position, PositionSet
from open_quant.target.reports import Report
from open_quant.target.risk_models import RiskModelTerm as Term
from open_quant.target.workflow_quote import WorkflowPosition, WorkflowPositionsResponse, SaveQuoteRequest
...
class GsPortfolioApi:
    ...
    def get_portfolios(cls,
        ...
        return OqSession.current._get(f'{url}&limit={limit}', cls=Portfolio)['results']

    @classmethod
    def get_portfolio(cls, portfolio_id: str) -> Portfolio:
        return OqSession.current._get('/portfolios/{id}'.format(id=portfolio_id), cls=Portfolio)

    @classmethod
    def get_portfolio_by_name(cls, name: str) -> Portfolio:
        ret = OqSession.current._get('/portfolios?name={}'.format(name))
    ...
    def create_portfolio(cls, portfolio: Portfolio) -> Portfolio:
        return OqSession.current._post('/portfolios', portfolio, cls=Portfolio)

    @classmethod
    def update_portfolio(cls, portfolio: Portfolio):
        return OqSession.current._put('/portfolios/{id}'.format(id=portfolio.id), portfolio, cls=Portfolio)

    @classmethod
    def delete_portfolio(cls, portfolio_id: str) -> dict:
        return OqSession.current._delete('/portfolios/{id}'.format(id=portfolio_id))
    ...
    def get_positions(cls, portfolio_id: str, start_date: dt.date = None, end_date: dt.date = None,
        ...
        res = OqSession.current._get(url)
    ...
    def get_positions_for_date(cls, portfolio_id: str, position_date: dt.date,
        ...
        position_sets = OqSession.current._get(url, cls=PositionSet)['results']
    ...
    def get_position_set_by_position_type(cls, positions_type: str, positions_id: str,
        ...
        results = OqSession.current._get(url, timeout=181)
    ...
    def get_latest_positions(cls, portfolio_id: str, position_type: str = 'close') -> Union[PositionSet, dict]:
        ...
        results = OqSession.current._get(url)['results']
    ...
    def get_position_dates(cls, portfolio_id: str) -> Tuple[dt.date, ...]:
        position_dates = OqSession.current._get('/portfolios/{id}/positions/dates'.format(id=portfolio_id))['results']
    ...
    def update_positions(cls,
        ...
        return OqSession.current._put(url, position_sets)
    ...
    def get_positions_data(cls,
        ...
        return OqSession.current._get(url)['results']

    @classmethod
    def update_quote(cls, quote_id: str, request: RiskRequest):
        return OqSession.current._put('/risk-internal/quote/save/{id}'.format(id=quote_id), request)

    @classmethod
    def save_quote(cls, request: RiskRequest) -> str:
        return OqSession.current._post('/risk-internal/quote/save', request)['results']

    @classmethod
    def update_workflow_quote(cls, quote_id: str, request: SaveQuoteRequest):
        headers = {'Content-Type': 'application/x-msgpack'}
        return OqSession.current._put('/risk-internal/quote/workflow/save/{id}'.format(id=quote_id), tuple([request]),
                                      request_headers=headers)

    @classmethod
    def save_workflow_quote(cls, request: SaveQuoteRequest) -> str:
        headers = {'Content-Type': 'application/x-msgpack'}
        return OqSession.current._post('/risk-internal/quote/workflow/save', tuple([request]),
                                       request_headers=headers)['results']

    @classmethod
    def share_workflow_quote(cls, request: SaveQuoteRequest) -> str:
        headers = {'Content-Type': 'application/x-msgpack'}
        return OqSession.current._post('/risk-internal/quote/workflow/share', tuple([request]),
                                       request_headers=headers)['results']
    ...
    def save_to_shadowbook(cls, request: RiskRequest, name: str) -> str:
        return OqSession.current._put(f'/risk-internal/shadowbook/save/{name}', request)['results']

    @classmethod
    def get_risk_models_by_coverage(cls, portfolio_id: str, term: Term = Term.Medium):
        return OqSession.current._get(f'/portfolios/{portfolio_id}/models?sortByTerm={term.value}')['results']

    @classmethod
    def get_reports(cls, portfolio_id: str) -> Tuple[Report, ...]:
        return OqSession.current._get('/portfolios/{id}/reports'.format(id=portfolio_id), cls=Report)['results']
    ...
    def schedule_reports(cls,
        ...
        return OqSession.current._post('/portfolios/{id}/schedule'.format(id=portfolio_id), payload)

    @classmethod
    def get_schedule_dates(cls,
                           portfolio_id: str,
                           backcast: bool = False) -> List[dt.date]:
        results = OqSession.current._get(f'/portfolios/{portfolio_id}/schedule/dates?backcast={str(backcast).lower()}')
    ...
    def get_custom_aum(cls,
        ...
        return OqSession.current._get(url)['data']
    ...
    def upload_custom_aum(cls,
        ...
        return OqSession.current._post(url, payload)
