...
from open_quant.target.assets import Asset as __Asset, AssetClass, AssetType, AssetToInstrumentResponse, TemporalXRef,\
    Position, EntityQuery, PositionSet, Currency, AssetParameters
from open_quant.target.assets import FieldFilterMap
from open_quant.target.common import Entitlements
from open_quant.target.reports import Report
from open_quant.errors import MqValueError
from open_quant.instrument import Instrument, Security
...
def _cached(fn):
    ...
    def wrapper(*args, **kwargs):
        if os.environ.get(ENABLE_ASSET_CACHING):
            ...
            k = cachetools.keys.hashkey(OqSession.current, *args, **kwargs)
            with metalock:
...
class OqAsset(__Asset):
    """GS Asset API object model for an asset object"""
    pass


class OqTemporalXRef(TemporalXRef):
    pass
...
class OqAssetApi:
    ...
    def get_many_assets(
            ...
            as_of: dt.datetime = None,
            limit: int = 100,
            return_type: Optional[type] = OqAsset,
            order_by: List[str] = None,
            **kwargs
    ) -> Union[Tuple[OqAsset, ...], Tuple[dict, ...]]:
        query = cls.__create_query(fields, as_of, limit, order_by=order_by, **kwargs)
        response = OqSession.current._post('/assets/query', payload=query, cls=return_type)
    ...
    def get_many_assets_data(
        ...
        response = OqSession.current._post('/assets/data/query', payload=query)
        return response['results']
    ...
    def resolve_assets(
        ...
        return OqSession.current._post('/positions/resolver', payload=query)
    ...
    def get_asset_xrefs(
            cls,
            asset_id: str
    ) -> Tuple[OqTemporalXRef, ...]:
        response = OqSession.current._get('/assets/{id}/xrefs'.format(id=asset_id))
        return tuple(OqTemporalXRef.from_dict(x) for x in response.get('xrefs', ()))

    @classmethod
    def put_asset_xrefs(cls, asset_id: str, xrefs: List[TemporalXRef]):
        return OqSession.current._put(f'/assets/{asset_id}/xrefs', payload=xrefs)
    ...
    def get_asset(
        ...
        return OqSession.current._get('/assets/{id}'.format(id=asset_id), cls=OqAsset)

    @classmethod
    def get_asset_by_name(cls, name: str) -> OqAsset:
        ret = OqSession.current._get('/assets?name={}'.format(name))
        ...
        if num_found == 0:
            ...
            return OqAsset.from_dict(ret['results'][0])

    @classmethod
    def create_asset(cls, asset: OqAsset) -> OqAsset:
        return OqSession.current._post('/assets', payload=asset, cls=OqAsset)

    @classmethod
    def delete_asset(cls, asset_id: str):
        return OqSession.current._delete(f'/assets/{asset_id}')

    @staticmethod
    def get_position_dates(asset_id: str) -> Tuple[dt.date, ...]:
        position_dates = OqSession.current._get(f'/assets/{asset_id}/positions/dates')['results']
    ...
    def get_asset_positions_for_dates(
        ...
        if periods > 0:
            ...
            for date in end_dates:
                ...
                try:
                    position_sets += OqSession.current._get(url)['positionSets']
    ...
    def get_or_create_asset_from_instrument(instrument: Instrument) -> str:
        asset = OqAsset(asset_class=instrument.asset_class,
                        type_=instrument.type,
                        name=instrument.name or '',
                        parameters=instrument.as_dict(as_camel_case=True))

        results = OqSession.current._post('/assets', asset)
        return results['id']
    ...
    def get_instruments_for_asset_ids(
        ...
        instrument_infos = OqSession.current._post('/assets/instruments', asset_ids, cls=AssetToInstrumentResponse)
    ...
    def update_asset_entitlements(asset_id: str, entitlements: Entitlements) -> dict:
        ...
        try:
            results = OqSession.current._put(url, payload=entitlements)
        except HTTPError as err:
    ...
    def get_reports(cls, asset_id: str) -> Tuple[Report, ...]:
        return OqSession.current._get(f'/assets/{asset_id}/reports', cls=Report)['results']
    ...
    def map_identifiers(
        ...
        results = OqSession.current._post('/assets/data/query', payload=query)
        ...
        for entry in results:
            ...
            if multimap:
                ...
                if key in out:
                    _logger.warning('%s: more than one mapping for %s', OqAssetApi.map_identifiers.__name__, key)
                out[key] = value
        return out
