...
from open_quant.target.common import Currency
...
class GsReportApi:
    ...
    def create_report(cls, report: Report) -> Report:
        return OqSession.current._post('/reports', report, cls=Report)

    @classmethod
    def get_report(cls, report_id: str) -> Report:
        return OqSession.current._get('/reports/{id}'.format(id=report_id), cls=Report)
    ...
    def get_reports(cls, limit: int = 100, offset: int = None, position_source_type: str = None,
        ...
        return OqSession.current._get(url, cls=Report)['results']

    @classmethod
    def update_report(cls, report: Report) -> dict:
        return OqSession.current._put('/reports/{id}'.format(id=report.id), report, cls=Report)

    @classmethod
    def delete_report(cls, report_id: str) -> dict:
        return OqSession.current._delete('/reports/{id}'.format(id=report_id))
    ...
    def schedule_report(cls, report_id: str, start_date: dt.date, end_date: dt.date, backcast: bool = False) -> dict:
        ...
        return OqSession.current._post('/reports/{id}/schedule'.format(id=report_id), report_schedule_request)

    @classmethod
    def get_report_status(cls, report_id: str) -> Tuple[dict, ...]:
        return OqSession.current._get('/reports/{id}/status'.format(id=report_id))

    @classmethod
    def get_report_jobs(cls, report_id: str) -> Tuple[dict]:
        return OqSession.current._get('/reports/{id}/jobs'.format(id=report_id))['results']

    @classmethod
    def get_report_job(cls, report_job_id: str) -> dict:
        return OqSession.current._get('/reports/jobs/{report_job_id}'.format(report_job_id=report_job_id))

    @classmethod
    def cancel_report_job(cls, report_job_id: str) -> dict:
        return OqSession.current._post('/reports/jobs/{report_job_id}/cancel'.format(report_job_id=report_job_id))
    ...
    def update_report_job(cls, report_job_id: str, status: str) -> dict:
        ...
        return OqSession.current._post('/reports/jobs/{report_job_id}/update'.format(report_job_id=report_job_id),
                                       status_body)
    ...
    def get_factor_risk_report_results(cls,
        ...
        return OqSession.current._get(url)
