...
from open_quant.api.risk import RiskApi
from open_quant.risk import RiskRequest
from open_quant.session import OqSession
from open_quant.target.risk import OptimizationRequest
...
class OqRiskApi(RiskApi):
    ...
    def _exec(cls, request: Union[RiskRequest, Iterable[RiskRequest]]) -> Union[Iterable, dict]:
        ...
        result, request_id = OqSession.current._post(cls.__url(request),
                                                     request,
    ...
    async def __get_results_poll(cls, responses: asyncio.Queue, results: asyncio.Queue, timeout: Optional[int] = None):
        ...
        while pending_requests or run:
            ...
            try:
                calc_results = OqSession.current._post('/risk/calculate/results/bulk', list(pending_requests.keys()))
    ...
    async def __get_results_ws(cls, responses: asyncio.Queue, results: asyncio.Queue, timeout: Optional[int] = None):
        ...
        while attempts < max_attempts:
            ...
            try:
                async with OqSession.current._connect_websocket('/risk/calculate/results/subscribe') as ws:
    ...
    def create_pretrade_execution_optimization(cls, request: OptimizationRequest) -> str:
        try:
            response = OqSession.current._post(r'/risk/execution/pretrade', request)
    ...
    def get_pretrade_execution_optimization(cls, optimization_id: str, max_attempts: int = 15):
        ...
        while attempts < max_attempts:
            ...
            try:
                results = OqSession.current._get(url)
