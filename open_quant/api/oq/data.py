...
from open_quant.api.data import DataApi
from open_quant.base import Base
from open_quant.data.core import DataContext, DataFrequency
from open_quant.data.log import log_debug, log_warning
from open_quant.errors import MqValueError
from open_quant.markets import MarketDataCoordinate
from open_quant.session import OqSession
from open_quant.target.common import MarketDataVendor, PricingLocation, Format
from open_quant.target.coordinates import MDAPIDataBatchResponse, MDAPIDataQuery, MDAPIDataQueryResponse, MDAPIQueryField
from open_quant.target.data import DataQuery, DataQueryResponse, DataSetCatalogEntry
from open_quant.target.data import DataSetEntity, DataSetFieldEntity
...
class GsDataApi(DataApi):
    ...
    def execute_query(dataset_id: str, query: Union[DataQuery, MDAPIDataQuery]):
        ...
        return OqSession.current._post('/data/{}/query'.format(dataset_id), **kwargs)
    ...
    def last_data(cls, query: Union[DataQuery, MDAPIDataQuery], dataset_id: str = None, timeout: int = None) \
        ...
        if getattr(query, 'marketDataCoordinates', None):
            result = OqSession.current._post('/data/coordinates/query/last', payload=query, **kwargs)
            return result.get('responses', ())
        else:
            result = OqSession.current._post('/data/{}/last/query'.format(dataset_id), payload=query, **kwargs)
    ...
    def get_coverage(
        ...
        body = OqSession.current._get(f'/data/{dataset_id}/coverage', payload=params)
    ...
    def create(cls, definition: Union[DataSetEntity, dict]) -> DataSetEntity:
        result = OqSession.current._post('/data/datasets', payload=definition)
        return result

    @classmethod
    def delete_dataset(cls, dataset_id: str) -> dict:
        result = OqSession.current._delete(f'/data/datasets/{dataset_id}')
        return result

    @classmethod
    def undelete_dataset(cls, dataset_id: str) -> dict:
        result = OqSession.current._put(f'/data/datasets/{dataset_id}/undelete')
        return result

    @classmethod
    def update_definition(cls, dataset_id: str, definition: Union[DataSetEntity, dict]) -> DataSetEntity:
        result = OqSession.current._put('/data/datasets/{}'.format(dataset_id), payload=definition, cls=DataSetEntity)
        return result
    ...
    def upload_data(cls, dataset_id: str, data: Union[pd.DataFrame, list, tuple]) -> dict:
        ...
        headers = None if 'us-east' in OqSession.current.domain else {'Content-Type': 'application/x-msgpack'}
        result = OqSession.current._post('/data/{}'.format(dataset_id), payload=data, request_headers=headers)
        return result
    ...
    def delete_data(cls, dataset_id: str, delete_query: Dict) -> Dict:
        ...
        """
        return OqSession.current._delete(f'/data/{dataset_id}', payload=delete_query, use_body=True)

    @classmethod
    def get_definition(cls, dataset_id: str) -> DataSetEntity:
        definition = cls.__definitions.get(dataset_id)
        if not definition:
            definition = OqSession.current._get('/data/datasets/{}'.format(dataset_id), cls=DataSetEntity)
            if not definition:
    ...
    def get_many_definitions(cls,
        ...
        body = OqSession.current._get('/data/datasets', payload=params, cls=DataSetEntity)
        results = scroll_results = body['results']
        total_results = body['totalResults']
    ...
    def get_catalog(cls,
        ...
        if len(query):
            return OqSession.current._get(f'/data/catalog?{query}', cls=DataSetCatalogEntry)['results']
        else:
            params = dict(filter(lambda item: item[1] is not None,
                                 dict(limit=limit, offset=offset, scroll=scroll, scrollId=scroll_id,
                                      enablePagination='true').items()))

            body = OqSession.current._get('/data/catalog', payload=params, cls=DataSetEntity)
            results = scroll_results = body['results']
            total_results = body['totalResults']
    ...
    def get_many_coordinates(
        ...
        results = OqSession.current._post('/data/mdapi/query', query)['results']
    ...
    def get_data_providers(cls,
        ...
        ** Usage **
        ...
        """
        response = availability if availability else OqSession.current._get(f'/data/measures/{entity_id}/availability')
    ...
    def get_market_data(cls, query, request_id=None) -> pd.DataFrame:
        OqSession.current: OqSession
        start = time.perf_counter()
        try:
            body = OqSession.current._post('/data/measures', payload=query)
    ...
    def get_types(dataset_id: str):
        results = OqSession.current._get(f'/data/catalog/{dataset_id}')
    ...
    def get_dataset_fields(
        ...
        **Examples**
        ...
        >>> from open_quant.api.oq.data import GsDataApi
        ...
        response = OqSession.current._post('/data/fields/query',
    ...
    def create_dataset_fields(
        ...
        **Examples**
        ...
        >>> from open_quant.api.oq.data import GsDataApi
        >>> from open_quant.target.data import DataSetFieldEntity
        >>> fields = [
        ...
        """
        params = {'fields': fields}
        response = OqSession.current._post('/data/fields/bulk', payload=params, cls=DataSetFieldEntity)
        return response['results']
    ...
    def update_dataset_fields(
        ...
        **Examples**
        ...
        """
        params = {'fields': fields}
        response = OqSession.current._put('/data/fields/bulk', payload=params, cls=DataSetFieldEntity)
        return response['results']
