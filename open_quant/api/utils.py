...
from open_quant.errors import MqUninitialisedError
from open_quant.session import OqSession


def handle_proxy(url, params):
    try:
        internal = OqSession.current.is_internal()
    except MqUninitialisedError:
        internal = False
    if internal or socket.getfqdn().split('.')[-2:] == ['oq', 'com']:
        try:
...
class ThreadPoolManager:
    ...
    def run_async(cls, tasks: List[Callable]) -> List:
        ...
        for i, task in enumerate(tasks):
            tasks_to_idx[cls.__executor.submit(cls.__run, OqSession.current, DataContext.current, task)] = i
